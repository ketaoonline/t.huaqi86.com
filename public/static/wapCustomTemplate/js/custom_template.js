...
var $cValue = {
	...
	liveTitle : {
		name : "直播标题",
		class_name : "live-navigation",
		input_class : "js-live-text",
		value : "『直播标题』",
		default_value : "",
		placeholder : "请输入直播标题"
	}
...
$(function() {
		...
		if(value.length){
			...
			if(validateDomainName(value)){
				...
				var href={
					...
					template_name: value,
					template_url: value,
					uniapp_template: '{"title":"自定义模板","url":"'+ value + '","is_tabbar":0}'
				};
				$(this).parent().parent().parent().find(".selected").text(value).attr("data-href",JSON.stringify(href)).css("display","inline-block");
	...
	$("input[name='whether_bold']").live('change',function(){
		...
		if($(this).val() == 1) {
			getCustom().find("h4[data-editable]").css("font-weight","bold");
			getCustom().find("p[data-editable]").css("font-weight","bold");
		}else{
			getCustom().find("h4[data-editable]").css("font-weight","normal");
			getCustom().find("p[data-editable]").css("font-weight","normal");
		}
	 ...
	 * [直播标题组件]->标题编辑
	 */
	$(".js-live-text").live("keyup",function(){

		if($(this).val().length>30){
			showTip("文字不得超出30个字符","warning");
			$(this).val($(this).val().substr(0,8));
		}

		bindLiveData();
		getCustom().find("h5 span").text($(this).val());
...
function loadData(){
	...
	if(!empty(template_data)){
		...
		for(var i=0;i<template_data.length;i++){
			...
			}else if(control_name == controlList.Live){
				additional_attr = "data-live='" + control_data.live + "' ";
			}else if(control_name == controlList.Discount){
				
			}else if(control_name == controlList.GroupPurchase){
				
			}else if(control_name == controlList.Pintuan){
...
function validation(){
	...
	if(!empty(control) && control.length){
		...
		for(var i=0;i<control.length;i++){
			...
			}else if(control[i].control_name == controlList.Live){

				var live = eval("(" + data.live + ")");
				
				if(live.title.length == 0){
					flag = false;		
					showTip($cValue.liveTitle.name + "不能为空","warning");
					$(".js-live-text").focus();			
				}

			}else if(control[i].control_name == controlList.Discount){			
				
			}else if(control[i].control_name == controlList.GroupPurchase){			
				
			}else if(control[i].control_name == controlList.Pintuan){			
	...
	switch(control_name) {
		...
		case controlList.Title:
			...
			if(!empty(control_data)){
				...
				if(title.whether_bold == 1){
					var whether_bold = "font-weight:bold";
				}
				style = 'style="color:' + title.text_color + ';text-align:' + align_array[title.text_align-1] + ';' + whether_bold + '   "';
		...
		case controlList.ShowCase:
			...
			break;
		case controlList.Live:		
			html += getLiveDefaultHTML(control_data);
			break;
		case controlList.Discount:		
			html += getDiscountDefaultHTML();
			break;
		case controlList.GroupPurchase:
			html += getGroupPurchaseDefaultHTML();
			break;
		case controlList.Pintuan:
			html += getPintuanDefaultHTML();
			break;
	}
...
function getGoodsListHTML(){
	...
	html += '<div class="control-edit-attribute">';
		...
		html += '<div class="js-show-buy-button" style="display:none;">';
		...
		html += '<div class="js-show-buy-button-style show-buy-button-style" style="display:none;">';
...
function getLinkHTML(href){
	...
	if(!empty(template_list) && template_list.length>0){
		...
		for(var i=0;i<template_list.length;i++){
			...
			for (var j=0;j<nav_list.length;j++) {				
				var nav = nav_list[j];				
				if(nav.template_url == __URL(APPMAIN + '/index')) {
					is_add = false;
					break;
				}
			}
		  /*if(is_add){
				nav_list.push({
					...
					template_url: __URL(APPMAIN + '/index'),
					uniapp_template: ''
				});
			}*/
	...
	if(!empty(href)){
		for (var j=0;j<nav_list.length;j++) {
			if(JSON.stringify(nav_list[j]) == JSON.stringify(href)){
				curr_link = nav_list[j];
				break;
			}
		}
		if(curr_link != null) href_name = curr_link.template_name;
	}
	
	if(!empty(getCustom().attr("data-href")) && empty(href)) href = JSON.stringify(curr_link);
	
	
	if(empty(href_name) && href != null){
		href_name = href.template_name;
		curr_link = href;
	}
	...
	var html = '<div class="control-edit link">';
		...
		if(href) html += "<span class='selected' style='display:inline-block;' title='" + href_name + "' data-href='" + JSON.stringify(curr_link) + "'>" + href_name + "</span>";
		...
		html += '<div class="dropdown">';
			...
			html += '<ul class="dropdown-menu js-link" role="menu" aria-labelledby="dLabel">';
				for(var link in nav_list){
					
					html += "<li data-href='" + JSON.stringify(nav_list[link]) + "'>" + nav_list[link].template_name + "</li>";
...
function getImgAdHTML(type=""){
	...
	for(var i=0;i<count;i++){
		...
		if(i!=0 && adv_show_type == $Default.advShowType[0]){
			...
			if(i >= 4){
				html += '<div class="control-edit img-ad notice-items">';
			}else{
			...
			html += '</div>';
			if(i >= 4){
				html += '<i class="brance-close" title="删除广告" data-i="'+ i +'"></i>';
			}
			
		html += '</div>';
	}
	if(adv_show_type == $Default.advShowType[1]){
		html += '<div class="control-edit plus-brange" style="padding: 5px;border: 1px solid #e5e5e5;background: #ffffff;"><span style="font-weight: bold;text-align: center;cursor: pointer;display: block;color: #126AE4;font-size: 25px;" class="add-notice">+</span></div>';
	}else{
		html += '<div class="control-edit plus-brange" style="display:none;padding: 5px;border: 1px solid #e5e5e5;background: #ffffff;"><span style="font-weight: bold;text-align: center;cursor: pointer;display: block;color: #126AE4;font-size: 25px;" class="add-notice">+</span></div>';
	}
	if(type == "delete"){
		$(".cont").html(html);
	}else{
...
$(".brance-close").live("click",function(){
	...
	getCustom().attr("data-img-ad", JSON.stringify(img_ad))
	$(this).parent().remove();
	$(".cont").html("");
	getImgAdHTML("delete");	
});
 ...
 * 添加一广告位
 */
$(".plus-brange .add-notice").live("click",function(){

	var i = $(".control-edit.img-ad").length;
	var add_notice = '<span class="add-notice">+</span>';
	$(this).remove();
	var html = '';
	html += '<div class="control-edit img-ad notice-items">';
		...
		html += '<div class="info">';
			html += getLinkHTML();
		html += '</div>';
		html += '<i class="brance-close" title="删除广告"></i>';
	html += '</div>';
	
	$(".control-edit.plus-brange").append(html + add_notice);
...
function bindTitleData(){
	...
	var json = {
		...
		href : !empty($(".control-edit.link>.selected").attr("data-href")) ? JSON.parse($(".control-edit.link>.selected").attr("data-href")) :  $Default.href
	};
...
function bindTextNavigationData(){
	...
	$(".control-edit.text-navigation-block").each(function(){
		...
		var obj = {
			...
			href : !empty($(this).find(".control-edit.link>.selected").attr("data-href")) ? JSON.parse($(this).find(".control-edit.link>.selected").attr("data-href")) :  $Default.href
		};
...
function bindFooterData(){
	...
	$(".control-edit.link>.selected").each(function(i){
		json[i].href = !empty($(this).attr("data-href")) ? JSON.parse($(this).attr("data-href")) : $Default.href;
	});
...
//直播
function getLiveDefaultHTML(control_data){
	var html = "";
	var title = $cValue.liveTitle.value;

	if(!empty(control_data)){
		var live = eval("(" + control_data.live + ")");
		title = live.title;
	}
	...
	html += '<div style="width:100%">';
		...
		html += '<div>';
			html += '<h5 data-editable="1">';
				html += '<span data-editable="1">' + title + '</span>';
			html += '</h5>';
		html += '</div>';
		
		html += '<div style="background:#fff">';
			html += '<div style="background:#fff;width:190px;height:100px;display:inline-block;border-radius: 15px;border:1px solid #ececec;margin:10px 0px">';
				html += '<div style="position:relative;height:100px;width:100px;border-radius: 15px;overflow: hidden;display:inline-block">';
					html += '<img src="'+ADMINIMG+'/adv_position/layout_1.png" style="width:100%;height:100%"/>';
					html += '<div style="line-height:16px;height:18px;position:absolute;top:5px;left:5px;border-radius:15px;background:rgba(0,0,0,0.2);">';
						html += '<div style="width:18px;height:18px;display:inline-block;overflow:hidden;border-radius:15px;float: left;">';
							html += '<img src="'+ADMINIMG+'/adv_position/layout_1.png" style="width:100%;height:100%"/>';
						html += '</div>';
						html += '<span style="color:#fff;font-size:10px;">直播中</span>';
					html += '</div>';
				html += '</div>';
				html += '<div style="display:inline-block;width: 83px;height: 100px;float:right">';
					html += '<span style="font-size:12px;font-weight:bold;margin-top: 5px;display: inline-block;">直播间名称</span>';
					html += '<div style="margin-top:5px;display:flex;align-items:flex-end;line-height:21px">';
						html += '<div style="width:20px;height:20px;border-radius:33px;display:inline-block;overflow:hidden">';
							html += '<img src="'+ADMINIMG+'/adv_position/layout_1.png" style="width:100%;height:100%"/>';
						html += '</div>';
						html += '<span style="display:inline-block;height:20px;font-size:12px">直播人员</span>';
					html += '</div>';
					html += '<div style="margin-top:5px">';
						html += '<div style="width:35px;height:35px;display:inline-block">';
							html += '<img src="'+ADMINIMG+'/adv_position/layout_1.png" style="width:100%;height:100%"/>';
						html += '</div>';
						html += '<div style="width:35px;height:35px;display:inline-block;margin-left: 5px;">';
							html += '<img src="'+ADMINIMG+'/adv_position/layout_1.png" style="width:100%;height:100%"/>';
						html += '</div>';
					html += '</div>';
				html += '</div>';
			html += '</div>';
			html += '<div style="background:#fff;width:120px;height:100px;display:inline-block;float:right;margin:10px 0px">';
				html += '<div style="height:100px;width:100px">';
					html += '<img src="'+ADMINIMG+'/adv_position/layout_1.png" style="width:100%;height:100%"/>';
				html += '</div>';
			html += '</div>';
		html += '</div>';
...
function getLiveHTML(){
	var html = "";
	var live = getCustom().attr("data-live");
	var title = $cValue.liveTitle.value;
	...
	html += '<div class="control-edit live-text-navigation-block">';

		html += getInputHTML("text:required",$cValue.liveTitle,title);		
...
function getLiveData(event_obj){
	var obj = { live : event_obj.attr("data-live") };
	return JSON.stringify(obj);
}
...
function bindLiveData(){
	var items = new Array();
	var json = {
		title : $(".control-edit.live-text-navigation-block .js-live-text").val()
	};
	getCustom().attr("data-live",JSON.stringify(json));
	
}

//限时折扣
function getDiscountDefaultHTML(){
	var html = "";
	
	html += '<div class="discount-box">';
		html += '<div class="discount-title">';
			html += '<span>限时折扣</span>';
			html += '<span><em>21</em>:<em>00</em>:<em>00</em></span>';
		html += '</div>';
		html += '<div>';
			html += '<div class="discount-goods">';
				html += '<div class="img-box"><img style="width:100%;height:100%" src="'+ADMINIMG+'/adv_position/layout_1.png"></div>'
				html += '<p class="goods-name">商品名称</p>';
				html += '<p class="goods-price"><span>￥99</span><span>￥120</span></p>';
			html += '</div>';
				html += '<div class="discount-goods">';
				html += '<div class="img-box"><img style="width:100%;height:100%" src="'+ADMINIMG+'/adv_position/layout_1.png"></div>'
				html += '<p class="goods-name">商品名称</p>';
...
function getGroupPurchaseDefaultHTML(){
	var html = "";
		
		html += '<div class="group-purchase">';
			html += '<div class="group-title">';
				html += '<span>团购专区</span>';
				html += '<span>查看更多</span>';
			html += '</div>';
			html += '<div class="group-goods">';
				html += '<div class="goods-img">';
					html += '<img style="width:100%;height:100%" src="'+ADMINIMG+'/adv_position/layout_1.png">';
				html += '</div>';
				html += '<p class="goods-name">商品名称</p>';
				html += '<p class="goods-price"><span>原价:￥99:00</span><span>团购价:￥85:00</span></p>';
				html += '<a>查看详情</a>';
			html += '</div>';
		html += '</div>';
...
function getGroupPurchaseHTML(){
	var html = "";
	
	return html;
}

function getGroupPurchaseData(){
	var obj = {};
	return JSON.stringify(obj);
}

function bindGroupPurchaseData(){
	var items = new Array();
	var json = {		
	};
}
...
function getPintuanDefaultHTML(){
	var html = "";
		
		html += '<div class="pintuan-box">';
			html += '<div class="pintuan-title">';
				html += '<span>拼团专区</span>';
				html += '<span>更多</span>';
			html += '</div>';
			html += '<div class="pintuan-goods">';
				html += '<div class="goods-box">';
					html += '<div class="goods-img">';
						html += '<img style="width:100%;height:100%" src="'+ADMINIMG+'/adv_position/layout_1.png">';
					html += '</div>';
					html += '<span>￥99.00</span>';
					html += '<span>￥80.00</span>';
				html += '</div>	';
				html += '<div class="goods-box">';
					html += '<div class="goods-img">';
						html += '<img style="width:100%;height:100%" src="'+ADMINIMG+'/adv_position/layout_1.png">';
...
function getPintuanHTML(){
	var html = "";
	
	return html;
}

function getPintuanData(){
	var obj = {};
	return JSON.stringify(obj);
}

function bindPintuanData(){
	var items = new Array();
	var json = {		
	};
}
