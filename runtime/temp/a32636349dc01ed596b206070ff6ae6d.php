...
<div>
	<nav class="ns-third-menu js-order-status-menu">
		<ul id="order_status_nav">
			<li <?php if($status==''): ?>class="selected"<?php endif; ?>" onclick="location.href='<?php if($order_type == ""): ?><?php echo __URL("http://t.huaqi86.com/index.php/admin/order/orderlist",''); else: ?><?php echo __URL("http://t.huaqi86.com/index.php/admin/order/orderlist",'order_type='.$order_type); endif; ?>'">全部</li>
...
$(function () {
	...
	$("#order_type").change(function () {
		...
		$.ajax({
			type : "post",
			url : "<?php echo __URL('http://t.huaqi86.com/index.php/admin/order/getOrderStatus'); ?>",
...
function LoadingInfo(page_index) {
	...
	$.ajax({
		type : "post",
		url : "<?php echo __URL('http://t.huaqi86.com/index.php/admin/order/orderlist'); ?>",
		data : {
...
// 							html += '</div>';
							...
							html += '<div class="product-infor">';
								...
								html += '<a href="'+__URL('http://t.huaqi86.com/index.php/goods/detail?goods_id='+goods_id)+'" target="_blank" >'+goods_name+'</a>';
						...
						html += '<td>';
							...
							if(	data["data"][i]["order_item_list"][0]['refund_status'] != 0) {
								...
								if (data["data"][i]["order_item_list"].length > 1) {
									html += '<a href="' + __URL('http://t.huaqi86.com/index.php/admin/order/orderrefunddetail?itemid=' + order_goods_id) + '" style="margin:5px 0 10px 0;display:block;text-align:center;">' + status_name + '</a>';
								} else {
									html += '<a href="' + __URL('http://t.huaqi86.com/index.php/admin/order/orderrefunddetail?itemid=' + order_goods_id) + '" style="margin:5px 0 10px 0;display:block;">' + status_name + '</a>';
								}
						...
						//买家信息
						html += '<td rowspan="'+row+'" style="text-align:center">';
							html += '<div class="cell"><a href="'+__URL('http://t.huaqi86.com/index.php/admin/member/memberdetail?member_id='+ data['data'][i]['buyer_id'])+'" class = "skip">'+data["data"][i]["user_name"]+'</a><br/>';
						...
						html += '<td rowspan="'+row+'" style="text-align:center;">';
							...
							if(order_type == '7'){
								html += '<a style="display:block;margin-bottom:5px;color:#4685fd;" href="'+__URL('http://t.huaqi86.com/index.php/admin/order/bargainOrderDetail?order_id='+order_id)+'">订单详情</a>';
							}else{
								html += '<a style="display:block;margin-bottom:5px;color:#4685fd;" href="'+__URL('http://t.huaqi86.com/index.php/admin/order/orderdetail?order_id='+order_id)+'">订单详情</a>';
							}
					...
					for(var j = 1; j < data["data"][i]["order_item_list"].length; j++){
						...
						html += '<div class="product-img"><img src="'+__IMG(pic_cover_micro)+'"></div>';
						html += '<div class="product-infor">';
						html += '<a class="name" href="'+__URL('http://t.huaqi86.com/index.php/goods/detail?goods_id='+goods_id)+'" target="_blank" >'+goods_name+'</a>';
						if(sku_name != null && sku_name != ''){
							html += '<p class="specification" style="margin-bottom: 0px;"><span style="color:#8e8c8c;font-size:12px;">'+sku_name+'</span></p>';
						}
						if(goods_code != null && goods_code != ''){
						...
						if(data["data"][i]["order_item_list"][j]['refund_status'] != 0){
							...
							html +='<br><a href="'+__URL('http://t.huaqi86.com/index.php/admin/order/orderrefunddetail?itemid='+order_goods_id)+'" style="margin:5px 0 5px 0;display:block;text-align:center;">'+status_name+'</a>';
...
function dataExcel(){
	...
	window.location.href=__URL("http://t.huaqi86.com/index.php/admin/order/orderDataExcel?start_date="+start_date+"&end_date="+end_date+"&user_name="+user_name+"&order_no="+order_no+"&order_status="+order_status+"&receiver_mobile="+receiver_mobile+"&payment_type="+payment_type+"&order_ids="+order_ids.toString() + "&order_type=" + order_type);
}
