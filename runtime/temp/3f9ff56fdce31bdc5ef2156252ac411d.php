...
<div class="set-style">
	<dl>
		<dt>是否开启分销:</dt>
		<dd>			
			<?php if($shop_config_info['is_distribution_enable'] == 1): ?> 
    ...
    <dl>
		<dt>分销商是否需要申请:</dt>
		<dd>			
			<?php if($shop_config_info['is_distribution_start'] == 1): ?> 
	...
	<dl>
		<dt>推广申请是否需要审核:</dt>
		<dd>			
			<?php if($shop_config_info['is_distribution_audit'] == 1): ?> 
	...
	<dl>
		<dt>批量设置为分销商:</dt>
		<dd>			
			<?php if($shop_config_info['is_distribution_set'] == 1): ?> 
	...
	<dl>
		...
		<dd>		
            <label class="radio inline normal">
            	<i class="radio-common <?php if($shop_config_info['distribution_use'] == 0): ?>selected<?php endif; ?>">
            		<input type="radio" name="distribution_use" value="0"  id="distribution_use_yes" <?php if($shop_config_info['distribution_use'] == 0): ?>checked<?php endif; ?>>
        		</i>
            	<span>使用利润</span>
            </label> 
		    <label class="radio inline normal">
		    	<i class="radio-common <?php if($shop_config_info['distribution_use'] == 1): ?>selected<?php endif; ?>">
		    		<input type="radio" name="distribution_use" value="1"  id="distribution_use_no"  <?php if($shop_config_info['distribution_use'] == 1): ?>checked<?php endif; ?>>
	    		</i>
		    	<span>销售价格</span>
		    </label> 
		</dd>
...
function modifyShopConfigIsDistribution(){
	...
	$.ajax({
		type:"post",
		url:"<?php echo __URL('http://t.huaqi86.com/index.php/nsfx/admin/Distribution/modifyShopConfigIsDistributionOrPromoterIsAudit'); ?>",
		data:{
			'is_open':is_open,
			'is_audit':is_audit,
			'is_start':is_start,
		...
		success:function (data) {
			if (data["code"] > 0) {
				showMessage('success', data["message"],"<?php echo __URL('http://t.huaqi86.com/index.php/nsfx/admin/Distribution/threeLevelDistributionConfig'); ?>");
			}else{
