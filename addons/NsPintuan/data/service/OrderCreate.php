...
class Ordercreate extends OrderCreateService
{
	...
	public function getGoodsPintuanPrice($data)
	{
		...
		if (!empty($tuangou_info['sku_json'])) {
		    $pintuan_sku_arr = json_decode($tuangou_info['sku_json'], true);
		    if (isset($pintuan_sku_arr['sku_id_'.$data['sku_id']])) {
    			return success([ "money" => $pintuan_sku_arr['sku_id_'.$data['sku_id']]['tuangou_price']]);
		    }
		} 
		return error([]);
	}
...
//                 }
				...
				//商品活动价格
				$sku_price_result = $this->getGoodsPintuanPrice($temp_array);
