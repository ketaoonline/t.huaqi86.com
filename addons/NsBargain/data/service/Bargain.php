...
//				]);
	...
	public function setBargainPartakeRecord($launch_id)
	{
		...
		if ($sum_bargain_money >= $bargain_money) {
			...
			$data['status'] = 2;
			$order_info = $this->orderCreate($bargain_launch_info);
	...
	public function getBargainLaunchList($page_index = 1, $page_size = 0, $condition = '', $order = 'launch_id desc', $field = '*')
	{
		...
		foreach ($list['data'] as $k => $v) {
			...
			if($v['order_id'] > 0){
				...
				$order_info = $order_model->getInfo(['order_id'=>$v['order_id']], "order_status");
				$list['data'][ $k ]['order_status'] = $order_info['order_status'];
			}else{
				$list['data'][ $k ]['order_status'] = "";
			}	
		}
		$user = new User();
...
// 	    }
	...
	public function bargainOperation()
	{
		...
		try {
			...
			$list = $bargain_launch->getQuery($condition_close_launch, 'launch_id,end_time, bargain_money,sku_id, launch_id, receiver_mobile, receiver_province, receiver_city, receiver_district, receiver_address, receiver_zip, receiver_name, uid, shipping_type, pick_up_id, goods_id', '');
