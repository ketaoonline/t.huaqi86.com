...
class GroupBuy extends BaseService
{
	...
	function delPromotionGroupBuy($group_id)
	{
		...
		try {
			$goods_promotion_model = new NsGoodsPromotionModel();
			$goods_promotion_model->destroy([ 'promotion_id' => array( "in", $group_id ), 'promotion_addon' => 'NsGroupBuy' ]);
	 ...
	 */
	public function getGroupBuyInfo($where = [], $field = '*', $ailas = 'a', $join = NULL, $data = NULL){
		if (!empty($join)) {
			$info = Db::table('ns_promotion_group_buy')->alias($ailas)->join($join)->where($where)->field($field)->find($data);
		} else {
			$info = Db::table('ns_promotion_group_buy')->where($where)->field($field)->find($data);
		}
		return $info;
	}
}