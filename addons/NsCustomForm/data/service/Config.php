...
namespace addons\NsCustomForm\data\service;
...
use addons\NsCustomForm\data\model\NsOrderCustomModel;
use data\service\BaseService;
use addons\NsCustomForm\data\model\NsOrderCustomDataModel;
...
class Config extends BaseService
{
	 ...
	 * 创建自定义表单
	 */
	public function saveOrderCustom($oc_name, $name_alias, $oc_id){
		$custom_model = new NsOrderCustomModel();
		...
		if($oc_id){
			$condition['oc_name'] = $oc_name;
			$condition['oc_id'] = [
					"neq",
					$oc_id
			];
			$info = $custom_model->getInfo($condition);
		}else{
			$info = $custom_model->getInfo(['name_alias'=>$name_alias]);
		}
		if ($info){
			return -2;
		}
		...
		if($oc_id){
			$res = $custom_model->save($data, ['oc_id'=>$oc_id]);
		}else{
			$data['create_time'] = time();
			$res = $custom_model->save($data);
		}
		return $res;
	}
	 ...
	 * 获取自定义表单列表
	 */
	public function getOrderCustomList($page_index = 1, $page_size = 0, $condition = "", $order = "", $field = "*"){
		$custom_model = new NsOrderCustomModel();
		
		$list = $custom_model->pageQuery($page_index, $page_size, $condition, $order, $field);
	 ...
	 * 删除自定义表单
	 */
	public function deleteOrderCoupon($oc_id){
		$custom_model = new NsOrderCustomModel();
		
		$res = $custom_model->destroy(['oc_id'=>$oc_id]);
		if($res){
			$custom_data = new NsOrderCustomDataModel();
			
			$re = $custom_data->destroy(["oc_id"=>$oc_id]);
		}
	 ...
	 * 设置表单字段
	 */
	public function saveUpdateCustomData($id, $field_name, $field_e_name, $oc_id, $is_fill, $field_type, $option_list){
		$custom_data = new NsOrderCustomDataModel();
		...
		$data = [
			...
			"option_text" => $option_list
		];
		
		if($id){
			$condition['field_e_name'] = $field_e_name;
			$condition['id'] = [
					"neq",
					$id
			];
			$info = $custom_data->getInfo($condition);
		}else{
			$info = $custom_data->getInfo(['field_e_name'=>$field_e_name]);
		}
		...
		if($id){
			$res = $custom_data->save($data, ["id"=>$id]);
		}else{
			$data['create_time'] = time();
			$res = $custom_data->save($data);
		}
	 ...
	 * 获取表单字段列表
	 */
	public function getCustomDataList($page_index = 1, $page_size = 0, $condition = "", $order = "", $field = "*"){
		$custom_data = new NsOrderCustomDataModel();
		
		$list = $custom_data->pageQuery($page_index, $page_size, $condition, $order, $field);
	 ...
	 * 删除表单字段
	 */
	public function deleteCustomData($id){
		$custom_data = new NsOrderCustomDataModel();

		$res = $custom_data->destroy(["id"=>$id]);
	 ...
	 * 获取自定义表单及字段
	 */
	public function getOrderCustomDataList($status = ""){
		$custom_model = new NsOrderCustomModel();
		$custom_data = new NsOrderCustomDataModel();
		
		if($status == 0){
			$list = $custom_model->getQuery(['status'=>0]);
		}else{
			$list = $custom_model->getQuery();
		}
			
		if(!empty($list)){
			foreach ($list as $key => $val){								
				$custom_list = $custom_data->getQuery(['oc_id'=>$val['oc_id']]);
				if (!empty($custom_list)){
					$list[$key]['custom_list'] = $custom_list;
				}else{
					$list[$key]['custom_list'] = "";
				}
			}
		}
	 ...
	 * 修改 自定义表单的状态
	 */
	public function setOrderCustomStatus($oc_id, $status){
		$custom_model = new NsOrderCustomModel();
		
		$res = $custom_model->save(['status'=>$status],['oc_id'=>$oc_id]);
	 ...
	 * 获取表单字段信息
	 */
	public function getOrderCustomDataDetail($id){
		$custom_data = new NsOrderCustomDataModel();
		
		$res = $custom_data->getInfo(["id"=>$id]);
