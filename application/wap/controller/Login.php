...
class Login extends BaseWap
{
		...
		if ($type == "WCHAT") {
		    if (request()->isMobile()) {
    			if (isWeixin()) {
        			...
        			Session::set('wchat_autologin_lock', null);
    				if($this->uid){
    				...
    				$this->error("当前为强制绑定模式，公众号内首次登陆需登陆已有账户或注册新账户");
    			}else{
	...
	function showUserQrcode()
	{
		...
		if(mb_strlen($member_info["nick_name"],'UTF8') > 5){
			$nick_name = mb_substr($member_info["nick_name"],0,5,'utf-8');
			$nick_name = $nick_name."...";
		}else{
		...
		@imagefttext($dests, $data['nick_font_size'], 0, $data['name_left'] * 2, $name_top_size, $bg, "public/static/font/Microsoft.ttf", $nick_name);
	 ...
	 * 拉取会员头像到本地
	 */
	public function pullOutUserImg()
	{
		$uid = request()->get('uid', '');
		$retval = $this->user->pullOutUserImg($uid);
		return $retval;
	}
}