...
class Pay extends Controller
{
	...
	public function __construct()
	{
		...
		$session_member_detail = session("niu_member_detail");
		if (!empty($session_member_detail)) {
			$uid = $session_member_detail['user_info']['uid'];
		}
		$this->assign("uid", $uid);
	...
	{  
		...
		$orderInfo = api("System.Order.orderInfo", ["out_trade_no" => $out_trade_no ]);
		
	    if ($zero1 > ($zero2 + ($this->shop_config['order_buy_close_time'] * 60)) && $orderInfo['data']['order_type'] != 6 && $this->shop_config['order_buy_close_time'] !=0) {
	        $this->error("订单已关闭");
	    }else{
	...
	public function payReturn()
	{
        ...
        $out_trade_no = empty($out_trade_no) ? $_POST['orderId'] : $out_trade_no;
