...
class Goods extends BaseWeb
{
	...
	public function detail($goods_id = '', $sku_id = '')
	{
		...
		$this->assign('current_province_id', Cookie::get('current_province_id'));
		$this->assign('current_city_id', Cookie::get('current_city_id'));
		$this->assign('current_district_id', Cookie::get('current_district_id'));
...
//			优先商品预售
	...
	public function lists()
	{
		...
		$coupon_type_id = request()->get('coupon_type_id', ''); // 优惠券id
	...
	public function topicDetail()
	{
		...
		if (!is_numeric($topic_id)) $this->error("没有获取到专题信息");
		
		$this->assign("topic_id", $topic_id);
		$detail = api("System.Goods.promotionTopicDetail", [ 'topic_id' => $topic_id ]);
		$detail = $detail['data'];
		if (empty($detail)) $this->error("没有获取到专题信息");
		
		$this->assign('info', $detail);
		$this->assign("title_before", $detail['topic_name']);
