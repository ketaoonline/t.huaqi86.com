...
class BaseWeb extends Controller
{
	...
	public function init()
	{
		...
		$controller = \think\Request::instance()->controller();
		$action = \think\Request::instance()->action();
		$seo_type = $controller.'/'.$action;
		//获取当前页SEO配置
		$seo_new = api("System.Config.seoConfigInfo", ['webpage_type' =>$seo_type]);
		//获取默认总配置
		$seo_all = api("System.Config.seoConfigInfo", ['webpage_type' =>"all"]);
		//效验数据返回新数组
		$seo = $this->seoArray($seo_new, $seo_all);
		...
		$this->assign('page_size', $system_config['show_num']);
	...
	public function seoArray($seo_new, $seo_all){
		if(empty($seo_new['data'])){
			$seo = $seo_all['data'];
		}else{
			$seo = $seo_new['data'];
			if($seo['seo_title'] == ""){
				$seo['seo_title'] = $seo_all['data']['seo_title'];
			}
			if($seo['seo_meta'] == ""){
				$seo['seo_meta'] = $seo_all['data']['seo_meta'];
			}
			if($seo['seo_desc'] == ""){
				$seo['seo_desc'] = $seo_all['data']['seo_desc'];
			}
			if($seo['seo_other'] == ""){
				$seo['seo_other'] = $seo_all['data']['seo_other'];
			}
		}
