...
class Member extends BaseController
{
	...
	public function pointlist()
	{
		if (request()->isAjax()) {
			...
			$from_type_list = MemberAccount::getMemberAccountRecordsNameList();
			$member = new MemberService();
			$money_sum = $member->getSelectBalanceSum(1);
	...
	public function accountlist()
	{
		if (request()->isAjax()) {
			...
			$this->assign('from_type_list', $from_type_list);
			$member = new MemberService();
			$money_sum = $member->getSelectBalanceSum(2);
			$this->assign("money_sum", $money_sum);
...
// 			//获取各项总值
	...
	public function memberLabelList()
	{
		if (request()->isAjax()) {
			...
			$list = $member->getMemberLabelList($page_index, $page_size, $condition, "create_time desc");
			return $list;
		} else {
	...
	public function giveCoupon(){
	    if (request()->isAjax()) {
	        ...
	        $member = new MemberService();
	        $res = $member->giveCoupon($uid, $couponId);
	        return $res;
	    }
	}	
}