...
class Promotion extends BaseController
{
	 ...
	 * 关闭优惠券
	 */
	public function couponTypeClose(){
		$coupon = new PromotionService();
		
		$coupon_type_id = request()->post("coupon_type_id", "");
		$status = request()->post("status", 0);
		
		$res = $coupon->updateCouponTypeClose($coupon_type_id, $status);
	...
	public function addUpdatePromotionGame()
	{
		if (request()->isAjax()) {
			...
			//是否在首页显示
			$is_index_show = request()->post('is_index_show','');
	...
	public function promotionGameInit()
	{
		...
		$coupon_condition = '(count > 0 AND start_time < ' . time() . ' AND end_time > ' . time() . ' AND term_of_validity_type = 0  AND status = 0)';
		$coupon_condition .= 'OR (term_of_validity_type = 1  AND status = 0)';
	...
	public function sendCouponTypeList()
	{
		if (request()->isAjax()) {
			$coupon = new PromotionService();
	 		$condition = '(count > 0  AND end_time > '.time().' AND term_of_validity_type = 0 AND shop_id = '.$this->instance_id.' AND status = 0)';
	        $condition .= 'OR (term_of_validity_type = 1 AND status = 0)';			
    ...
    public function goodsSelectList(){
        if (request()->isAjax()) {
            ...
            $is_limit_open_sku = request()->post("is_limit_open_sku", "");
	        ...
	        $result = $goods->getSearchGoodsList($page_index, $page_size, $condition, "create_time desc", "*", $is_limit_open_sku);
            return $result;
        } else {
            // 商品分组
            ...
            $sku_id_array = request()->get("sku_id_array", "");
            $this->assign("sku_id_array", $sku_id_array);
	 ...
	 * 关闭赠品
	 */
	public function setGiftStatus(){
		$gift_id = request()->post("gift_id", "");
		
		$topic = new PromotionService();
		
		$res = $topic->setGiftStatus($gift_id);
