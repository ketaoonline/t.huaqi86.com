...
class Goods extends BaseController
{
	...
	public function goodsList()
	{
		...
		if (request()->isAjax()) {
			...
			if(cookie("page_index") && cookie("update_goods") == 1){
				$page_index = cookie("page_index");
				cookie("page_index", "");
				cookie("update_goods", 0);
			}else{
				cookie("page_index", $page_index);
			}
			...
			foreach ($result['data'] as $k => $v) {
				...
				if (!empty($v['group_id_array'])) {
					...
					foreach ($goods_group_id as $key => $val) {
    					...
    					array_push($goods_group_array, ['group_id' => $val, 'group_name' => $goods_group_info['group_name']]);
					}
					$result["data"][ $k ]['goods_group_array'] = $goods_group_array;
				}
			}
			return $result;
		} else {
		...
		if ($goods_id > 0) {
			...
			if (!empty($goods_info)) {
				...
				if (addon_is_exit('Nsfx')) {
                    ...
                    $fx_commission_config = $commission_config->getGoodsCommissionRate($goods_id);
                    $this->assign('fx_commission_config', $fx_commission_config);
				}
	...
	public function getDialogGoodsList(){
		...
		$list = $promotion->getDialogGoodsList($goods_id_array, $discount_id, $type, $sku_id);
	 ...
	 * 取消商品推荐和标签
	 */
	public function cancelLabelRecommend(){
	    if (request()->isAjax()) {
	        $goods_id = request()->post('goods_id', '');
	        $type = request()->post('type', '');
	        $value = request()->post('value', '');
	        $goods = new GoodsService();
	        $res = $goods->cancelLabelRecommend($goods_id, $type, $value);
	        return AjaxReturn($res);
	    }
	}
}