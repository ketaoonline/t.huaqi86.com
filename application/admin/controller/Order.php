...
use addons\NsCustomForm\data\service\Config;
...
// 		}
		...
		if (request()->isAjax()) {
			...
			$order_express_no = request()->post('order_express_no','');
			//var_dump($order_express_no);
			...
			if(!empty($order_express_no)){
			    $condition['order_express_no'] = $order_express_no;
			}
	...
	public function orderDetail()
	{
		...
		if(addon_is_exit('NsCustomForm')){
			...
			$custom_list = $custom_config->getOrderCustomDataList();
	...
	public function orderDeliveryData()
	{
		...
		// 订单商品项
		$order_goods_list = $order_query->getOrderGoods($order_id, 1);
	...
	public function orderDataExcel()
	{
		...
		if ($order_status != '') {
			...
			if ($order_status == 1) {
				// 订单状态为待发货实际为已经支付未完成还未发货的订单
				$condition['no.shipping_status'] = 0; // 0 待发货
		...
		if (!empty($payment_type)) {
			$condition['no.payment_type'] = $payment_type;
		}
		if (!empty($user_name)) {
			$condition['no.receiver_name'] = $user_name;
		}
		if (!empty($order_no)) {
			$condition['no.order_no'] = $order_no;
		}
		if (!empty($receiver_mobile)) {
			$condition['no.receiver_mobile'] = $receiver_mobile;
		}
		$condition['no.shop_id'] = $this->instance_id;
		
		$order_query = new OrderQuery();
		$list = $order_query->getOrderGoodsListExcel(1, 0, $condition, 'no.create_time desc');
...
//			)
	...
	public function orderInvoiceDataExcel()
	{
		$xlsName = "订单发票列表";
		$xlsCell = array(
				array(
		...
		foreach ($list as $k => $v) {
			if($v['buyer_invoice']){
				$invoice = explode('$', $v['buyer_invoice']);					
				$invoice_detil['order_no'] = $v['order_no'];
				$invoice_detil['user_name'] = $v['user_name'];				
				$invoice_detil['pay_money'] = $v['pay_money'];				
				$invoice_detil['invoice_title'] = $invoice[0] ? $invoice[0]: "--";					
				$invoice_detil['taxpayer_identification_number'] = $invoice[2] ? $invoice[2]: "--";					
				$invoice_detil['opening_Bank'] = $invoice[3] ? $invoice[3]: "--";					
				$invoice_detil['bank_account'] = $invoice[4] ? $invoice[4]: "--";					
				$invoice_detil['business_address'] = $invoice[5] ? $invoice[5]: "--";					
				$invoice_detil['business_tel'] = $invoice[6] ? $invoice[6]: "--";					
				$invoice_detil['invoice_text'] = $invoice[1] ? $invoice[1]: "--";					
				$invoice_detil["create_time"] = getTimeStampTurnTime($v["create_time"]); // 创建时间	
				$list_data[] = $invoice_detil;
			}		
		}
		dataExcel($xlsName, $xlsCell, $list_data);
	}
}