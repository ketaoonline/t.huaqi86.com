...
//				'web_gov_record_url' => $web_gov_record_url
	...
	public function seoConfig()
	{
		...
		if (request()->isAjax()) {
			...
			$seo_id = request()->post("seo_id", '');
			//$retval = $Config->SetSeoConfig($this->instance_id, $seo_title, $seo_meta, $seo_desc, $seo_other);
			$retval = $Config->updateSeoConfig($seo_id, $seo_title, $seo_meta, $seo_desc, $seo_other);
			return AjaxReturn($retval);
		} else {
			$seo_id = request()->get("seo_id", "");
			
			$shopSet = $Config->getSeoConfigInfo(['seo_id'=>$seo_id]);
	...
	public function copyrightinfo()
	{
		...
		if (request()->isAjax()) {
			...
			//网站的备案信息
			$web_icp = request()->post("web_icp", '');
			$web_gov_record = request()->post("web_gov_record", '');
			$web_gov_record_url = request()->post("web_gov_record_url", '');
			...
			$updateWebSite = $Website->updateWebSite($data);
			...
			if ($updateWebSite > 0 && $retval > 0) {
				$copyrightinfo = 1;
			} else {
				$copyrightinfo = 0;
			}
			return AjaxReturn($copyrightinfo);
		} else {
	...
	public function addShopNavigation()
	{
		...
		if (request()->isAjax()) {
			...
			$uniapp_data = request()->post('uniapp_data', '');
	...
	public function registerAndVisit()
	{
		...
		if (request()->isAjax()) {
			...
			$res_two = $config_service->setRegisterAndVisit($this->instance_id, $is_register, $register_info, $name_keyword, $pwd_len, $pwd_complexity, $terms_of_service, $is_requiretel, $is_use, $is_login);
	...
	public function wapCategoryDisplay()
	{
		...
		$this->assign('info', $info);
		$this->assign('is_info', json_decode($info, true));
...
// 	        $this->assign("goods_id_array", $goods_id_array);
	...
	public function webFloat()
	{
		...
		if (request()->isAjax()) {
			...
			$nav_icon = request()->post("nav_icon", '');
			$is_show = request()->post('is_show', '');
			$show_type = request()->post('show_type', 'once');
			$show_num = request()->post('show_num', 1);
			$data = array(
				'shop_id' => 0,
				'nav_title' => $nav_title,
				'nav_url' => $nav_url,
	...
	public function wapEntranceManage()
	{
		...
		$list = $config->getWapEntranceList(['port'=> $port]);
	...
	public function editWapEntrance()
	{
		if (request()->isAjax()) {
			...
			$port = request()->post('port', 1);
			$config = new WebConfig();
			$res = $config->editWapEntrance($data, $port);
			return AjaxReturn($res);
		}
	}
	...
	public function systemConfig()
	{
		...
		if (request()->isAjax()) {
			...
			$res = $config->setSystemConfig($data);
			return AjaxReturn($res);
		}
		//$this->infrastructureChildMenu(2);
		$info = $config->getSystemConfig(0);
		$this->assign('info', $info);
		return view($this->style . 'Config/systemConfig');
	}
	...
	public function appletFloat()
	{
		...
		if (request()->isAjax()) {
			...
			$retval = $shop->setAppletFloatConfig($data);
			return AjaxReturn($retval);
		} else {
			$retval = $shop->getAppletFloatConfig();
			$this->assign('retval', $retval);
			return view($this->style . "Config/appletFloat");
		}
	}
	...
	public function rechargeSetting(){
    	...
    	if(request()->isAjax()) {
    	    ...
    	    $res = $config_service->setSystemRechargeConfig($data);
    	    return AjaxReturn($res);
    	} else {    	
    	    $info = $config_service->getSystemRechargeConfig($this->instance_id);
            $this->assign('info', $info['value']);
    	    ...
    	    $this->assign("child_menu_list", $child_menu_list);
    	    return view($this->style . "Config/rechargeSetting");
    	}
	}
	...
	public function seoConfigList()
	{
		if(request()->isAjax()) {
			...
			$list = $config_service->getSysSeoConfigList();
			
			return $list;
			
		}else{
			return view($this->style . 'Config/seoConfigList');
		}
