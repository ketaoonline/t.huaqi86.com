...
class Wchat extends BaseController
{
	 ...
	 * 新增     微信小程序流程图页面
	 */
	public function appletProcess(){
	    $config = new Config(); 
	    $applet = new AppletVersion();
	    $applet_list = $applet->getAppletList();
	    
	    /**
	     * 获取官网最新小程序列表以及商场数据库小程序最新列表是否一致
	     */
	    // var_dump($applet_list['data']);die;
	    if(!empty($applet_list['data'])){
	        foreach($applet_list['data'] as $k=>$v){
	            $web_res = $this->getwebnewApplet($v['module_mark']);
	            if(!empty($web_res)){
	                $local_applet = $applet -> getlocApplet($web_res['version_name'],$web_res['module_mark']);
	                if(!empty($local_applet)){
	                    $applet_list['data']['is_set'] = 1;//已存在
	                    break;
	                }else{
	                    $applet_list['data']['is_set'] = 0;//不存在
	                }
	            }
	        }
	    }
	    ...
	    $this->assign('applet_list', $applet_list['data']);
	    return view($this->style . 'Wchat/appletProcess');
	}
...
// 		    array(
// 		        'url' => "wchat/appletConfig",
// 		        'menu_name' => "小程序配置",
// 		        "active" => 1
// 		    ),
// 		    array(
// 		        'url' => "wchat/appletRelease",
// 		        'menu_name' => "小程序发布",
// 		        "active" => 0
// 		    ),
// 	    );
// 		$this->assign('child_menu_list', $child_menu_list);
	...
	public function channelPromotion(){
	    return view($this->style . "Wchat/channelPromotion");
	}
	 ...
	 * 小程序发布
	 */
	public function appletRelease(){
	    $applet = new AppletVersion();
	    $applet_list = $applet->getAppletList();
	    //var_dump($applet_list);die;
// 	    $child_menu_list = array(
// 	        array(
// 	            'url' => "wchat/appletConfig",
// 	            'menu_name' => "小程序配置",
...
// 	    $this->assign('child_menu_list', $child_menu_list);
	   ...
	   // var_dump($applet_list['data']);die;
	    if(!empty($applet_list['data'])){
	        foreach($applet_list['data'] as $k=>$v){
	            ...
	            if(!empty($web_res)){
	                $local_applet = $applet -> getlocApplet($web_res['version_name'],$web_res['module_mark']);
	                if(!empty($local_applet)){
	                    $applet_list['data'][$k]['product_info']['is_set'] = 1;//已存在
	                }else{
	                    $applet_list['data'][$k]['product_info']['is_set'] = 0;//不存在
	                }
	            } 
	        }
	    }
	    //var_dump($applet_list['data']);die;
	    $this->assign('applet_list', $applet_list['data']);
	    return view($this->style . 'Wchat/appletRelease');
	}
	
	/**
	 * 下载小程序
	 */
	public function downloadApplet(){
	    $applet = new AppletVersion();
	    if (request()->isAjax()){
	        $mark = request()->post('mark', '');
	        $applet = new AppletVersion();
	        $list = $applet->getAppletVersionList(['applet_module_mark' => $mark]);
	        return $list;
	    }
	    
	    $mark = request()->get('mark', '');
	    $type = request()->get('type', 'co');
	    $this->assign('mark', $mark);
	    $this->assign('type', $type);
	    /**
	     * 查询官网最新版小程序是否存在于商城数据库中,如不存在，则添加。
	     */
	    $res_data = $this->getwebnewApplet($mark);
	    //var_dump($res_data);die;
	    if(!empty($res_data)){
	        $local_applet = $applet -> getlocApplet($res_data['version_name'],$res_data['module_mark']);
            if(empty($local_applet)){
                $data = array(
                    "version_name"=>$res_data['version_name'],
                    "mark"=>$res_data['module_mark'],
                    "is_look"=>0,
                    "creattime"=>time()
                );
                $insert_applet_loc = $applet->addlocApplet($data);
                
            }
	    }
	    return view($this->style . 'Wchat/downloadApplet');
	}
	
	/**
	 * 获取官网最新小程序版本列表
	 */
	public function getwebnewApplet($mark){
	    $applet = new AppletVersion();
	    $niuweb_applet = $applet->getAppletVersionList(['applet_module_mark' => $mark]);
	    $num = count($niuweb_applet['data']);
	    $result = $niuweb_applet['data'][$num -1];
	    return $result;
	}
	...
	public function download(){
	    $mark = request()->post('mark', '');
	    $type = request()->post('type', 'co');
	    $release = request()->post('release', '');

	    $applet = new AppletVersion();
        $data = $applet->getAppletVersionUpgradeInfo(["type" => $type, "applet_module_mark" => $mark, "last_version_release" => $release]);
        return $data;
	}
	...
	public function toDownload(){
	    ...
	    $applet = new AppletVersion();
	    $dowmload_url = $applet->download(['token' => $token]);
	    $this->redirect($dowmload_url);
	}
}