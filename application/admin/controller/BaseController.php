...
class BaseController extends Controller
{
	...
	public function checkNewVersion(){
	    $date = date("Y-m-d");
	    $version_list = Session::get("version_list".$date);
        if($version_list == null){
            $up_model = new \data\service\Version();
            $sign = file_get_contents('./cert.key');
            $result = $up_model->getUpgradeVersion($sign);
            if(!empty($result)){
                $version_list = $result["data"];
            }else{
                $version_list = [];
            }
            Session::set("version_list".$date, $version_list);
        }
	...
	public function init()
	{
		...
		if($check_auth == 0 && $this->action == "index"){
			
			$module_url = $this->auth->getUserGroupJump($this->moduleid);
			
			$redirect = __URL(__URL__ . '/' . ADMIN_MODULE . "/".$module_url);
			$this->redirect($redirect);
			...
			if (!request()->isAjax()) {
				...
				// 面包屑导航
				$bread_crumb = $this->auth->breadCrumb($this->moduleid);
