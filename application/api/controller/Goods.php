...
class Goods extends BaseApi
{
	...
	public function goodsBrandList()
	{
		...
		$page_size = isset($this->params['page_size']) ? $this->params['page_size'] : $this->system_config['show_num'];
		$condition = isset($this->params['condition']) ? $this->params['condition'] : [];
		$order = isset($this->params['order']) ? $this->params['order'] : 'sort ' . $this->system_config['sort_type'];
	 ...
	 * 根据IP定位查询当前商品的运费
	 */
	public function shippingFeeByIpWeb()
	{
	    $goods_id = isset($this->params['goods_id']) ? $this->params['goods_id'] : 0;
	    $goods_sku_list = isset($this->params['goods_sku_list']) ? $this->params['goods_sku_list'] : '';
	    $province_id = isset($this->params['province_id']) ? $this->params['province_id'] : 0;
	    ...
	    if (!empty($goods_id)) {
	        ...
	        if (empty($province_id) && empty($city_id) && empty($district_id)) {
	           return $this->outMessage('运费', null);
	        }
	...
	public function goodsComments()
	{
		...
		foreach ($goodsEvaluationList['data'] as $v) {
			$member_info = $memberService->getMemberImage($v["uid"]);
			$v["user_img"] = $member_info['member_img'];
			if ($v['member_name'] == $member_info['user_tel'] && $member_info['user_tel']){
				$tel = substr($v["member_name"],2, 7);
				$v["member_name"] = str_replace($tel,"*****",$v["member_name"]);
			}
		}
...
//		}
	...
	public function couponList()
	{
		...
		$condition = '(count > 0  AND end_time > ' . time() . ' AND is_show = 1 AND term_of_validity_type = 0 AND status = 0)';
		$condition .= 'OR (term_of_validity_type = 1 AND is_show = 1 AND status = 0)';
	...
	private function showMemberCouponQrcode($coupon_type_id)
	{
		...
		if ($is_uniapp == 1) {
			...
			if ($is_h5 == 1) {
				$url = $domain . '/pages/goods/coupon_receive/coupon_receive?coupon_type_id=' . $coupon_type_id . '&source_uid=' . $uid;
				$path = $upload_path . '/uniapp_coupon_' . $coupon_type_id . '_' . $uid . '.png';
				if (!file_exists($path)) {
					getQRcode($url, $upload_path, "uniapp_coupon_" . $coupon_type_id . '_' . $uid);
				}
			} else {
				$wchat_oauth = new WchatOauth();
				$page = 'pages/goods/coupon_receive/coupon_receive';
				$scene = "coupon_type_id-" . $coupon_type_id;
				$path = 'upload/uniapp_goods_coupon/' . date('Ymd', time()) . '/';
				$name = 'temp_qrcode_coupon_' . $coupon_type_id . '_u' . $uid;
				$path = $wchat_oauth->getAppletQrcode($scene, $page, true, $path, $name);
			}
	...
	public function goodsPurchaseRestriction()
	{
		...
		$num = isset($this->params['num']) ? $this->params['num'] : 0;
		$cart_id = isset($this->params['cart_id']) ? $this->params['cart_id'] : '';
		$goods_purchase_restriction = $goods->getGoodsPurchaseRestrictionForCurrentUser($goods_id, $num, '', $cart_id);
	...
	public function goodsListByConditions()
	{
		...
		$page_size = $this->get('page_size', $this->system_config['show_num']);
		$coupon_type_id = $this->get('coupon_type_id', '');
		...
		if ($order != "") {
			if ($order != "sales" && $order != "is_new" && $order != "price") {
				// 非法参数进行过滤
				$orderby = "ng.sort {$this->system_config['sort_type']}, ng.goods_id desc";
			} else {
				if ($order == "is_new") {
		...
		$params['coupon_type_id'] = $coupon_type_id;
	...
	private function getGoodsListByConditions($params)
	{
		...
		//根据优惠券查找商品
		if($params['coupon_type_id']){
			$goodsid_str = [];
			$promotion_service = new PromotionService();
			$coupon_goods = $promotion_service->getCouponGoodsList($params['coupon_type_id']);
		
			foreach ($coupon_goods as $key => $val){
				$goodsid_str[] = $val['goods_id'];
			}
	...
	public function goodsList()
	{
		...
		$order = $this->get('order', 'ng.sort ' . $this->system_config['sort_type'] . ',ng.create_time desc');
		if (!empty($condition) && !is_array($condition) && json_decode($condition)) {
			$condition = json_decode($condition, true);
		}
	...
	public function goodsCategoryBlockPc()
	{
		...
		$block_list = $shop->getGoodsFloorList(1, 0, [ 'is_use' => 1, 'pc_template' => $pc_template ], 'sort ' . $this->system_config['sort_type'], '*');
		...
		if ($block_list['data']) {
			...
			foreach ($block_list['data'] as $k => $v) {
				...
				$this->assign('rand_num', mt_rand(1001, 9999));
	 ...
	 * 获取小程序的分类展示
	 */
	public function getAppletCategoryDisplay(){
		$web_config = new ConfigService();
...
// 			}
	...
	public function getGoodsPoster()
	{
		$title = '获取商品海报';
		$goods_id = request()->post('goods_id', 0);
		$promotion_info = isset($this->params['promotion_info']) ? $this->params['promotion_info'] : "";
		$goods = new GoodsService();
		
		if (!empty($promotion_info)) {
			$promotion_info = json_decode($promotion_info, true);
		}
		$result = $goods->createGoodsPoster($goods_id, $this->uid, $promotion_info);
		if ($result == -50) {
	 ...
	 * 获取商品点击量
	 */
	public function getSelectClicks(){
		$title = '获取商品点击量';
		
		$goods_id = isset($this->params['goods_id']) ? $this->params['goods_id'] : "";
		
		$goods = new GoodsService();
		$clicks = $goods->getSelectClicks($goods_id);
		
		return $this->outMessage($title, $clicks);
	}
}