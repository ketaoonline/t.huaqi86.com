...
use think\Request;
use think\Session;
use addons\NsCustomForm\data\service\Config as CustomConfig;
...
class Member extends BaseApi
{
	...
	public function footprint()
	{
		...
		if (!empty($data['category_id'])) {
			...
			$category_list = $goods_category->getGoodsCategoryTree($data['category_id']);
			$cate_arr = [];
			$cate_arr[] = (int) $data['category_id'];
			foreach ($category_list as $key => $val) {
				$cate_arr[] = $val['category_id'];
				foreach ($val['child_list'] as $k => $v) {
					$cate_arr[] = $v['category_id'];
				}
			}
	...
	public function withdrawRecordList()
	{
		...
		$page_size = isset($this->params['page_size']) ? $this->params['page_size'] : PAGESIZE;
		$order = isset($this->params['order']) ? $this->params['order'] : 'ask_for_date desc';
	...
	public function shareContents()
	{
		...
		switch ($flag) {
			case "goods":
				...
				if (!empty($this->uid)) {
					if (!empty($group_id)) {
						if ($this->is_uniapp) $share_url = $domain . '/promotionpages/pintuan/detail/detail?goods_id=' . $goods_id . '&group_id=' . $group_id . '&source_uid=' . $this->uid;
						else $share_url = __URL(__URL__ . '/wap/goods/detail?goods_id=' . $goods_id . '&group_id=' . $group_id . '&source_uid=' . $this->uid);
					} elseif (!empty($bargain_id)){
						if ($this->is_uniapp) $share_url = $domain . '/promotionpages/bargain/detail/detail?goods_id=' . $goods_id . '&bargain_id=' . $bargain_id . '&source_uid=' . $this->uid;
						else $share_url = __URL(__URL__ . '/wap/goods/detail?goods_id=' . $goods_id . '&bargain_id=' . $bargain_id . '&source_uid=' . $this->uid);
					} else {
						if ($this->is_uniapp) $share_url = $domain . '/pages/goods/detail/detail?goods_id=' . $goods_id . '&source_uid=' . $this->uid;
						else $share_url = __URL(__URL__ . '/wap/goods/detail?goods_id=' . $goods_id . '&source_uid=' . $this->uid);
					}
				} else {
					if (!empty($group_id)) {
						if ($this->is_uniapp) $share_url = $domain . '/promotionpages/pintuan/detail/detail?goods_id=' . $goods_id . '&group_id=' . $group_id;
						else $share_url = __URL(__URL__ . '/wap/goods/detail?goods_id=' . $goods_id . '&group_id=' . $group_id);
					} elseif (!empty($bargain_id)){
						if ($this->is_uniapp) $share_url = $domain . '/promotionpages/bargain/detail/detail?goods_id=' . $goods_id . '&bargain_id=' . $bargain_id;
						else $share_url = __URL(__URL__ . '/wap/goods/detail?goods_id=' . $goods_id . '&bargain_id=' . $bargain_id);
					} else {
						if ($this->is_uniapp) $share_url = $domain . '/pages/goods/detail/detail?goods_id=' . $goods_id;
						else $share_url = __URL(__URL__ . '/wap/goods/detail?goods_id=' . $goods_id);
					}
				}
				...
				if (count($goods_detail["img_list"]) > 0) {
					if (strstr($goods_detail["img_list"][0]["pic_cover_mid"], "http://") || strstr($goods_detail["img_list"][0]["pic_cover_mid"], "https://")) {
						$share_logo = $goods_detail["img_list"][0]["pic_cover_mid"];
					} else {
						$domain = str_replace('index.php/', '', Request::instance()->domain()); 
						$domain = str_replace('api.php/', '', $domain); 
						$share_logo = $domain . '/' . $goods_detail["img_list"][0]["pic_cover_mid"]; // 用商品的第一个图片
					}
				}
				break;
			case "bargain":
				...
				if ($this->is_uniapp) $share_url = $domain . '/promotionpages/bargain/launch/launch?launch_id=' . $launch_id;
				...
				if (strstr($bargain_goods["goods_picture"], "http://") || strstr($bargain_goods["goods_picture"], "https://")) {
					$share_logo = $bargain_goods["goods_picture"];
				} else {
					$domain = str_replace('index.php/', '', Request::instance()->domain()); 
					$domain = str_replace('api.php/', '', $domain);
					$share_logo = $domain . '/' . $bargain_goods["goods_picture"]; 
				}
				break;
			case "shop":
				//首页分享
				if (!empty($this->uid)) {
					if ($this->is_uniapp) $share_url = $domain . '/pages/index/index/index?source_uid=' . $this->uid;
					else $share_url = __URL(__URL__ . '/wap/index?source_uid=' . $this->uid);
				} else {
					if ($this->is_uniapp) $share_url = $domain . '/pages/index/index/index';
					else $share_url = __URL(__URL__ . '/wap/index');
				}
			...
			case "fx_shop_qrcode":
				...
				if (!empty($this->uid)) {
					...
					if ($this->is_uniapp) $share_url = $domain . '/distributionpages/user_shop_goods/user_shop_goods?source_uid=' . $this->uid;
					...
					if (!empty($user_info['user_headimg'])) {
						if (strstr($user_info['user_headimg'], "http：//") || strstr($user_info['user_headimg'], "https：//")) {
							$share_logo = $user_info['user_headimg'];
						} else {
							$share_logo = $domain . '/' . $user_info['user_headimg'];
						}
					} else {
						$share_logo = $domain . '/template/wap/' . $use_wap_template['value'] . '/public/img/member_default.png';
					}
				}
				break;
			case "cms":
	...
	public function modifyMobile()
	{
		...
		$info = $notice_service->getNoticeTemplateOneDetail(0, "sms", "bind_mobile", "user");
		
		
		if ($mobile_is_use == 1 && $info['is_enable'] == 1) {
...
//			$hook = runhook('Notify', 'bindMobile', $params);
	...
	public function checkEmail()
	{
		...
		if ($exist) {
			return $this->outMessage($title, $exist, 0, '邮箱已存在');
		} else {
			return $this->outMessage($title, $exist, 0, '邮箱不存在');
		}
	}
	...
	public function getOrderCustom()
	{
		...
		$list = $custom_config->getOrderCustomDataList(0);
		
		return $this->outMessage("获取自定义表单信息", $list);
	}
	
	/**
	 * 查询订单自定义数据
	 */
	public function getOrderCustomFormData()
	{
		$order_query = new OrderQuery();
		
		$order_id = isset($this->params['order_id']) ? $this->params['order_id'] : "";
		
		$custom_detail = $order_query->getOrderCustomFormData($order_id);
		
		return $this->outMessage("获取自定义表单信息", $custom_detail);
	}
	...
	public function getCustomDataList()
	{
		...
		$oc_id = isset($this->params['oc_id']) ? $this->params['oc_id'] : "";
		
		$condition['oc_id'] = $oc_id;
		
		$list = $custom_config->getCustomDataList(1, 0, $condition, "", "*");
		
		return $this->outMessage("获取表单字段", $list);
	}
	...
	public function getSmsConfig()
	{
		...
		return $this->outMessage("短信配置", $noticeMobile);
	}
	...
	public function getNoticeTemplateOneDetail()
	{
		...
		$template_code = isset($this->params['template_code']) ? $this->params['template_code'] : "";
		$notify_type = isset($this->params['notify_type']) ? $this->params['notify_type'] : "";
		
		$info = $notice_service->getNoticeTemplateOneDetail(0, $template_type, $template_code, $notify_type);
		
		return $this->outMessage("", $info);
	}
	
	/**
	 * 修改手机
	 */
	public function modifyMobileAPI()
	{
		$title = '修改手机';
		if (empty($this->uid)) {
			return $this->outMessage($title, [ 'code' => -9999, 'message' => '无法获取会员登录信息' ]);
		}
		
		$member = new MemberService();
		$mobile = isset($this->params['account']) ? $this->params['account'] : '';
		
		$is_bin_mobile = $member->memberIsMobile($mobile);
		if ($is_bin_mobile) {
			return $this->outMessage($title, [ 'code' => -1, 'message' => '该手机号已存在' ]);
		}
	 ...
	 * 修改邮箱
	 */
	public function modifyemailAPI()
	{
		$title = '修改邮箱';
		$member = new MemberService();
		if (empty($this->uid)) {
		...
		$email = isset($this->params['account']) ? $this->params['account'] : '';
		
		$is_bin_email = $member->memberIsEmail($email);
		if ($is_bin_email) {
			return $this->outMessage($title, [ 'code' => -1, 'message' => '该邮箱已存在' ]);
		}
	...
	public function getUserId(){
	    $title = '获取用户id';
	    return $this->outMessage($title, $this->uid);
	}
}