...
class Login extends BaseApi
{
	...
	public function loginConfig()
	{
		...
		$code_config['value']['curr_err_num'] = $this->getLoginErrorNum();
		// 注册配置
	...
	public function login()
	{
		...
		if ($retval > 0) {
		    ...
		    $curr_error_num = $this->getLoginErrorNum() + 1;
		    $data['error_num'] = $curr_error_num;
		    $this->setLoginErrorNum($curr_error_num);
			return $this->outMessage($title, $data);
		}
	}
	...
	public function emailRegister()
	{
		...
		if ($notice[0]['is_use'] == 1) {
			...
			if ($email_code != $param) {
				return $this->outMessage($title, -1, -1, '邮箱证码错误');
			}
		}
...
//		$result = runhook("Notify", "forgotPassword", $params);
     ...
     */
    private function getLoginErrorNum()
    {
        $ip = request()->ip();
        $error_num = Session::get($ip . 'error_num');
        if ($error_num == null) {
            $error_num = 0;
            Session::set($ip . 'login_error_num', 0);
        }
        return $error_num;
    }
     ...
     */
    private function setLoginErrorNum($error_num)
    {
        $ip = request()->ip();
        Session::set($ip . 'login_error_num', $error_num);
    }
     ...
     * @param unknown $error_num
     */
    private function clearLoginErrorNum()
    {
        $ip = request()->ip();
        Session::delete($ip . 'login_error_num');
    }
    ...
    public function mobileLoginAPI()
    {
        ...
        $mobile = $this->get('account', '');
        $sms_captcha = $this->get('code', '');
       
        $send = new Send();
        $res_json = $send->checkDynamicCode();
        $check_res = json_decode($res_json, true);
        if ($check_res['code'] != 0) return $this->outMessage($title, $check_res);
    
        $member = new Member();
        $retval = $member->login($mobile, '');
        if ($retval == USER_ERROR) {
            $source_uid = request()->get('source_uid', ''); // 来源用户id
            if ($source_uid && addon_is_exit('Nsfx')) {
                $_SESSION['source_uid'] = $source_uid;
            }
            // 手机号未注册
     ...
     * 手机注册(uniapp使用)
     */
    public function mobileRegisterAPI()
    {
        $title = "手机号注册";
    ...
    public function passwordResetAPI(){
        ...
        if (empty($password)) return $this->outMessage($title, ['code' => -1, '缺少必需参数password']);
