...
class Distribution extends BaseApi
{
	...
	{
		$title = '分销商信息';
		$uid = !empty($this->params['uid']) ? $this->params['uid'] : $this->uid;
		if (empty($uid)) {
			return $this->outMessage($title, null, '-9999', "无法获取会员登录信息");
		}
		$nfx_promoter = new NfxPromoter();
		$promoter_detail = $nfx_promoter->getPromoterDetailByUid($uid, 'promoter_shop_name, background_img');
		$promoter_detail['user_headimg'] = '';
		if (empty($promoter_detail)) {
			return $this->outMessage($title, null, -50, '非法操作');
		}
		$user = new User();
		$user_info = $user->getUserInfoByCondition(['uid' => $uid], 'user_headimg');
		if (!empty($user_info['user_headimg'])) $promoter_detail['user_headimg'] = $user_info['user_headimg'];
		return $this->outMessage($title, $promoter_detail);
	}
	...
	public function applyWithdraw()
	{
		...
		$retval = $nfx_user->addNfxCommissionWithdraw($this->instance_id, $withdraw_no, $this->uid, $bank_account_id, $cash, $type);
		return $this->outMessage($title, $retval);
	}
	...
	function userFxQrcode()
	{
		...
		if ($is_applet == 1) {
            ...
            if ($is_uniapp) {
            	$scene = 'sourceuid_' . $uid;
	            $page = 'distributionpages/user_shop_goods/user_shop_goods';
            } else {
	            $scene = 'sourceid_' . $uid;
    		...
    		if ($is_uniapp) {
    			$path = getQRcode($domain . '/distributionpages/user_shop_goods/user_shop_goods?source_uid=' . $this->uid, $upload_path, 'uni_shop_goods_qrcode_' . $this->uid);
    		} else {
		...
		if ($is_applet == 1) {
		    $upload_applet_path = "upload/qrcode/promote_qrcode/user_fx/applet";
		    if (!is_dir($upload_applet_path)) {
		        $mode = intval('0777', 8);
		        mkdir($upload_applet_path, $mode, true);
		    }
			if ($is_uniapp) $img_path = 'upload/qrcode/promote_qrcode/user_fx/applet/uni_shop_goods_qrcode_' . $this->uid . '.png';
			else $img_path = 'upload/qrcode/promote_qrcode/user_fx/applet/shop_goods_qrcode_' . $this->uid . '.png';
		} else {
			if ($is_uniapp) $img_path = 'upload/qrcode/promote_qrcode/user_fx/uni_shop_goods_qrcode_' . $this->uid . '.png';
			else $img_path = 'upload/qrcode/promote_qrcode/user_fx/shop_goods_qrcode_' . $this->uid . '.png';
		}
	...
	function showUserQecode($path, $thumb_qrcode, $user_headimg, $shop_logo, $user_name, $data, $create_path)
	{
		...
		if ($create_path == "") {
		    ...
		    if (file_exists($create_path)) unlink($create_path);
	...
	public function distributionGoodsList()
	{
		...
		$uid = !empty($this->params['uid']) ? $this->params['uid'] : $this->uid;// 推广员uid
