...
class Config extends BaseApi
{
	...
	public function addonIsExit()
	{
		...
		$data['is_exit_sign'] = addon_is_exit('NsMemberSign'); // 是否存在签到
		$data['is_exit_virtual_goods'] = addon_is_exit('NsVirtualGoods'); // 是否存在虚拟商品
	 ...
	 * 检测插件是否存在（单个插件）
	 */
	public function singleAddonIsExit()
	{
		$addon = $this->get('addon', '');
		$is_exit = false;
		if (!empty($addon)) {
			$is_exit = (bool) addon_is_exit($addon);
		}
		return $this->outMessage('检测插件是否存在（单个插件）', $is_exit);
	}
	...
	{
		$web_config = new ConfigService();
		$port = isset($this->params['port']) ? $this->params['port'] : 1;
		
		$data = $web_config->getWapEntranceList([ 'status' => 1, 'port'=> $port]);
	...
	{
		$config = new ConfigService();
		$data = $config->getSystemConfig($this->instance_id);
		return $this->outMessage('获取系统配置', $data);
	}
	...
	public function recharge(){
	    $config = new ConfigService();
	    $rechargeConfig = $config->getSystemRechargeConfig($this->instance_id);
	    return $this->outMessage("获取系统充值配置", $rechargeConfig);
	}
	
	/**
	 * 获取当前页面的seo
	 */
	public function seoConfigInfo(){
		
		$webpage_type = isset($this->params['webpage_type']) ? $this->params['webpage_type'] : "";
		
		$config = new ConfigService();
		
		$info = $config->getSeoConfigInfo(['webpage_type' => $webpage_type]);
		
		return $this->outMessage("获取SEO配置", $info);
	}
}