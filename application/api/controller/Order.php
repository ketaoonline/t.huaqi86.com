...
//		}
	...
	public function refundDetail()
	{
		...
		if ($detail['refund_type'] > 0) {
			...
			if (!empty($type_info['type_name'])) {
				if ($type_info['type_name'] != '支付宝') {
					$detail['refund_type_name'] = str_replace('支付', '', $type_info['type_name']);
				} else {
					$detail['refund_type_name'] = $type_info['type_name'];
				}
			}
		}
	...
	public function applyOrderCustomer()
	{
		...
		if ($retval > 0) {
			...
			return $this->outMessage($title, $retval, $retval, "该虚拟商品已经核销");
		} else {
	...
	public function getWapVirtualGoodsShare()
	{
		...
		$domain = $this->get('domain', "domain");
		
		if ($is_uniapp == 1) {
			$path = $this->getUniAppVirtualQecode($vg_id, $is_h5, $domain);
			if ($path == -50) {
	 ...
	 * 制作uni-app小程序核销二维码
	 */
	function getUniAppVirtualQecode($virtual_goods_id, $is_h5, $domain)
	{
		$title = '制作核销二维码';
		if (empty($this->uid)) {
			return $this->outMessage($title, null, '-9999', "无法获取会员登录信息");
		...
		// 查询并生成二维码
		$upload_path = "upload/uniapp_qrcode/virtual_qrcode";
		if (!file_exists($upload_path)) {
			mkdir($upload_path, 0777, true);
		}
		if ($is_h5 == 1) {
			$url = $domain . '/pages/verification/goods/goods?vg_id=' . $virtual_goods_id;
			$path = $upload_path . '/uniapp_verification_goods_' . $virtual_goods_id . '.png';
			if (!file_exists($path)) {
				getQRcode($url, $upload_path, '/uniapp_verification_goods_' . $virtual_goods_id);
			}
		} else {
			$wchat_oauth = new WchatOauth();
			$page = 'pages/verification/goods/goods';
			$scene = "virtual_goods_id-" . $virtual_goods_id;
			$path = $wchat_oauth->getAppletQrcode($scene, $page, true);
		}
		return $path;
	}
	 ...
	 * 获取uni-app小程序自提码
	 */
	public function getUniAppPickupQecode()
	{
		$title = '获取自提二维码';
		if (empty($this->uid)) {
			return $this->outMessage($title, null, '-9999', "无法获取会员登录信息");
		...
		if (!empty($pickup_info['picked_up_code'])) {
			$upload_path = "upload/uniapp_qrcode/order_pickup_code_qrcode";
			if (!file_exists($upload_path)) {
				mkdir($upload_path, 0777, true);
			}
			if ($is_h5 == 1) {
				$url = $domain . '/pages/order/pickup_toexamine/pickup_toexamine?order_id=' . $order_id;
				$path = $upload_path . '/uniapp_pickup_code_' . $order_id . '.png';
				if (!file_exists($path)) {
					getQRcode($url, $upload_path, '/uniapp_pickup_code_' . $order_id);
				}
			} else {
				$wchat_oauth = new WchatOauth();
				$scene = "order_id-" . $order_id;
				$page = 'pages/order/pickup_toexamine/pickup_toexamine';
				$path = $wchat_oauth->getAppletQrcode($scene, $page, true);
			}
			if ($path == -50) {
				return $this->outMessage($title, '', -50, '商家未配置小程序');
	...
	public function cancelAfterSale(){
		...
		$cancle_order = $order_refund->orderGoodsCancelAfterSale($order_id, $order_goods_id);
		return $this->outMessage($title, $cancle_order);		
	}
}
