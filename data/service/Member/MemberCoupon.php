...
class MemberCoupon extends BaseService
{
	 ...
	 * @param unknown $get_type 1.订单  2.首页领取  3.商品详情页领取 4.营销活动获取5.商家发放
	 */
	public function userAchieveCoupon($uid, $coupon_type_id, $get_type)
	{
		...
		if ($max_fetch != 0 && ($fetched_count >= $max_fetch) && ($get_type != 1 && $get_type != 4  && $get_type != 5)){
	...
	public function getUserCouponList($type = '', $shop_id = '')
	{
		...
		$coupon_list = $coupon->alias('nc')
			...
			->field('nc.coupon_id, nc.coupon_code, nc.uid, nc.money, nc.state, nc.get_type, nc.fetch_time, nc.use_time, nc.start_time, nc.end_time, nct.coupon_name, nct.at_least, nct.range_type, nct.term_of_validity_type, nct.fixed_term, nct.coupon_type_id')->where($condition)->order('nc.fetch_time desc')->select();
