	...
	public function editGoods($data)
	{
		...
		try {
			...
			if (addon_is_exit('Nsfx')) {
				if (!empty($data['fxSetting'])) {
					$commission_config = new NfxCommissionConfig();
					$data['fxSetting']['page'] = 1;
					$data['fxSetting']['type'] = 0;
					$data['fxSetting']['from_type'] = 'goods_ids';
					$data['fxSetting']['goods_ids'] = $goods_id;
					$commission_config->goodsDistributionSetting($data['fxSetting']);
				}
			}
...
//		$goods_detail["purchase_num"] = $num;
	...
	public function getBasisGoodsDetail($param = [])
	{
		...
		if (empty($cache)) {
			...
			if ($goods_detail['goods_attribute_id'] != 0) {
				...
				$goods_attribute_list = $goods_attribute_model->getQuery([
					'goods_id' => $param['goods_id']
				], 'attr_id, goods_id, shop_id, attr_value_id, attr_value, attr_value_name, sort', 'sort asc');
				$goods_detail['goods_attribute_list'] = $goods_attribute_list;
			} else {
				$goods_detail['goods_attribute_name'] = '';
				$goods_detail['goods_attribute_list'] = array();
	...
	public function getBusinessGoodsInfo($goods_detail, $param)
	{
		...
		if (empty($cache)) {
			...
			// 针对拼团进行特殊处理
			if (!empty($goods_detail['promotion_detail']['pintuan'])) {
				if (!empty($goods_detail['promotion_detail']['pintuan']['data']['sku_list'])) {
					$pintuan_sku_list = $goods_detail['promotion_detail']['pintuan']['data']['sku_list'];
					array_multisort(array_column($pintuan_sku_list, 'tuangou_price'), SORT_ASC, $pintuan_sku_list);
					$pintuan_sku_list = array_values($pintuan_sku_list);
					// 按价格升序排列 取拼团价最低的一条为默认sku
					$param['sku_id'] = $pintuan_sku_list[0]['sku_id'];
				}
			}
...
//			}
		...
		// 查询商品会员价
		if ($goods_member_discount == 100 || empty($goods_member_discount)) {
				...
				foreach ($v["value"] as $m => $t) {
					...
					foreach ($goods_detail['sku_list'] as $a => $b) {
						...
						if ($b['sku_id'] == $goods_detail['sku_id']) {
							$curr_sku_index = $a;
							$value = $spec_list[ $k ]["value"][ $m ]['spec_id'] . ":" . $spec_list[ $k ]["value"][ $m ]['spec_value_id'];
							$spec_list[ $k ]["value"][ $m ]['selected'] = in_array($value, explode(";", $b['attr_value_items'])) ? true : false;// && $b['stock'] > 0
	 ...
	 * 按照添加查询特定分页列表
	 */
	public function getSearchGoodsList($page_index = 1, $page_size = 0, $condition = '', $order = 'create_time desc', $field = '*', $is_limit_open_sku = 1)
	{
		...
		foreach ($result['data'] as $k => $v) {
			...
			if ($is_limit_open_sku == 0) {
				$goods_sku_list = $goods_sku_model->pageQuery(1, 0, [ "goods_id" => $v['goods_id'] ], "create_date desc", "*");
				foreach ($goods_sku_list['data'] as $key => $val) {
					if ($val['sku_name'] == "") {
						$goods_sku_list['data'][ $key ]['sku_name'] = $v['goods_name'];
					} else {
						$goods_sku_list['data'][ $key ]['sku_name'] = $val['sku_name'];
					}
					
					$sku_pic_info = array();
					$sku_pic_info['pic_cover'] = '';
					if ($val['picture'] != 0) {
						$sku_pic_info = $picture->get($val['picture']);
					} else {
						$sku_pic_info = $picture->get($v['picture']);
					}
					$goods_sku_list['data'][ $key ]['picture_info'] = $sku_pic_info;
				}
				$result['data'][ $k ]['sku_list'] = $goods_sku_list;
			}
	 ...
	 */
	public function getGoodsPurchaseRestrictionForCurrentUser($goods_id, $num = 0, $flag = "", $cart_id = "")
	{
		$res = array(
			"code" => 1,
			"message" => "允许购买",
	...
	public function getBackStageGoodsList($page_index = 1, $page_size = 0, $condition = '', $order = 'ng.sort desc')
	{
		...
		$viewObj = $goods_model->alias("ng")
			->join('sys_album_picture ng_sap', 'ng_sap.pic_id = ng.picture', 'left')
			->field("ng.goods_id,ng.goods_name,ng.promotion_price,ng.market_price,ng.goods_type,ng.stock,ng.introduction,ng.max_buy,ng.state,ng.is_hot,ng.is_recommend,ng.is_new,ng.sales,ng.shipping_fee,ng_sap.pic_cover_micro, ng_sap.pic_cover_small,ng.code,ng.create_time,ng.QRcode,ng.price,ng.real_sales,ng.sort,ng.group_id_array,ng.is_virtual");
	...
	public function updateGoodsSkuBatch($goods_sku_arr, $goods_id)
	{
		...
		try {
			...
			if (count($goods_sku_arr) == 1) {
				$goods_data['cost_price'] = $goods_sku_arr[1]['cost_price'];
			}
			$goods_model->save($goods_data, [ 'goods_id' => $goods_id ]);
			
			$this->modifyGoodsPromotionPrice($goods_id);
			
			hook("goodsSkuupdateSuccess", $goods_sku_arr);
			//编辑商品后清除商品详情缓存
			Cache::tag("niu_goods")->set("getBasisGoodsDetail" . $goods_id, null);
			Cache::tag("niu_goods")->set("getBusinessGoodsInfo_" . $goods_id, null);
			$goods_model->commit();
		...
		if ($params['uid'] > 0) {
			...
			if ($count == 0 || empty($count)) {
				$data = array(
					'buyer_id' => $params['uid'],
					'shop_id' => $this->instance_id,
					'shop_name' => !empty($params['shop_name']) ? $params['shop_name'] : $web_info['title'],
					'goods_id' => $params['goods_id'],
	...
	public function getCart($uid)
	{
		...
		if (!empty($cart_goods_list)) {
			foreach ($cart_goods_list as $k => $v) {
				...
				// 查询商品是否存在团购活动
				$group_buy_info = null;
				if (addon_is_exit('NsGroupBuy')){
					$group_buy = new GroupBuy();
					$where = [
						'npgb.goods_id' => ['=', $v['goods_id']],
						'npgb.start_time' => ['<', time()],
						'npgb.end_time' => ['>', time()]
					];
					$group_buy_info = $group_buy->getGroupBuyInfo($where, 'npgb.max_num,npgb.min_num,npgbl.group_price', 'npgb', [ ['ns_promotion_group_buy_ladder npgbl', 'npgb.group_id = npgbl.group_id', 'LEFT'] ]);
				}
				if (!empty($group_buy_info)) {
					$cart_goods_list[ $k ]['max_buy'] = $group_buy_info['max_num'];
					$cart_goods_list[ $k ]['min_buy'] = $group_buy_info['min_num'];
				}
				
				$num = $v['num'];
				if ($cart_goods_list[ $k ]['max_buy'] != 0 && $cart_goods_list[ $k ]['max_buy'] < $v['num']) {
					$num = $cart_goods_list[ $k ]['max_buy'];
				}
				if ($sku_info['stock'] < $num) {
					$num = $sku_info['stock'];
				}
				// 商品最小购买数大于现购买数
				if ($cart_goods_list[ $k ]['min_buy'] > 0 && $num < $cart_goods_list[ $k ]['min_buy']) {
					$num = $cart_goods_list[ $k ]['min_buy'];
				}
				// 商品最小购买数大于现有库存
				if ($cart_goods_list[ $k ]['min_buy'] > $sku_info['stock']) {
					unset($cart_goods_list[ $k ]);
					// 更新cookie购物车
					$this->deleteCart($v['cart_id']);
					continue;
				...
				if (!empty($group_buy_info)) {
					$cart_goods_list[ $k ]["promotion_price"] = round($group_buy_info['group_price'], 2);
					$cart_goods_list[ $k ]["price"] = $group_buy_info['group_price'];
				} else {
					// 阶梯优惠后的价格
	...
	public function getGoodsCoupon($goods_id)
	{
		...
		foreach ($coupon_type_id_list as $key => $val){
			$coupon = new NsCouponModel();
			$surplus_coupon = $coupon->getCount([
					'coupon_type_id' => $val['coupon_type_id'],
					'state' => 0
			]);
			if ($surplus_coupon == 0) {
				unset($coupon_type_id_list[ $key ]);
			} else {
				$received_num = 0;
				if (!empty($this->uid) && $val['max_fetch'] != 0) {
				...
				$coupon_type_id_list[ $key ]['received_num'] = $received_num;
			}
		}
	 ...
	 * 获取商品阶梯优惠
	 */
	public function getGoodsLadderPreferential($condition, $order = "", $filed = "*", $cart_id = "")
	{
		$nsGoodsLadderPreferential = new NsGoodsLadderPreferentialModel();
		$list = $nsGoodsLadderPreferential->pageQuery(1, 0, $condition, $order, $filed);
		foreach ($list['data'] as $key => $val){
			$list['data'][$key]['cart_id'] = $cart_id;
		}
		return $list["data"];
	}
	 ...
	 * @param string $uid
	 */
	public function createGoodsPoster($goods_id, $uid = "", $promotion_info = [])
	{
		try {
			$data = $this->getGoodsPosterData($goods_id, $uid, $promotion_info);
			...
			$is_uniapp = Session::get('is_uniapp') == 1 ? 1 : 0;
			
			if ($is_uniapp == 1) {
				$image_config = [
					'width' => 740,
					'height' => 1100,
					'path' => 'upload/uniapp_goods_poster/' . date('Ymd', time()) . '/',
			...
			// 对logo按高度来进行等比率放大缩小
			$new_logo_width = round($logo_width * round((80 / $logo_height), 2));
			...
			$file_res = $upload->fileStoreHaiBao($image_config['image_path']);
			
			if($file_res['code'] == 1){
				$image_config['image_path'] = $file_res['path'];
			}
	 ...
	 * @param unknown $uid
	 */
	public function getGoodsPosterData($goods_id, $uid, $promotion_info = [])
	{
		$user = new UserModel();
		$config = new Config();
		$website = new WebSite();
		...
		$is_applet = Session::get('is_applet') == 1 ? 1 : 0;
		$is_uniapp = Session::get('is_uniapp') == 1 ? 1 : 0;
		$is_h5 = Session::get('is_h5') == 1 ? 1 : 0;
		$domain = Session::get('domain') != "" ? Session::get('domain') : "";
		...
		if ($is_uniapp == 1) {
			...
			if ($is_h5 == 1) {
				$qrcode_config['url'] = $domain . '/pages/goods/detail/detail?goods_id=' . $goods_id;
			}
			
			if (!empty($promotion_info)) {
				switch ($promotion_info['promotion_flag']) {
					case "bargain":
						//砍价详情
						if ($is_h5 == 1) {
							$qrcode_config['url'] = $domain . '/promotionpages/bargain/detail/detail?goods_id=' . $goods_id . '&bargain_id=' . $promotion_info['bargain_id'];
						} else {
							$qrcode_config['url'] = 'promotionpages/bargain/detail/detail';
							$qrcode_config['scene'] = 'goods_id-' . $goods_id . '&bargain_id-' . $promotion_info['bargain_id'];
						}
						break;
					case "group_buy":
						//团购详情
						if ($is_h5 == 1) {
							$qrcode_config['url'] = $domain . '/promotionpages/groupbuy/detail/detail?goods_id=' . $goods_id;
						} else {
							$qrcode_config['url'] = 'promotionpages/groupbuy/detail/detail';
							$qrcode_config['scene'] = 'goods_id-' . $goods_id;
						}
						break;
					case "pintuan":
						//拼团详情
						if ($is_h5 == 1) {
							$qrcode_config['url'] = $domain . '/promotionpages/pintuan/detail/detail?goods_id=' . $goods_id . '&group_id=' . $promotion_info['group_id'];
						} else {
							$qrcode_config['url'] = 'promotionpages/pintuan/detail/detail';
							$qrcode_config['scene'] = 'goods_id-' . $goods_id . '&group_id-' . $promotion_info['group_id'];
						}
						break;
				}
			}
	 ...
	 * @param unknown $value
	 */
	public function cancelLabelRecommend($goods_id, $type, $value)
	{
		$goods_model = new NsGoodsModel();
		$data = [];
		if ($type == 'recommend') {
			...
			$goods_info = $goods_model->getInfo([ 'goods_id' => $goods_id ], 'group_id_array');
			if (!empty($goods_info['group_id_array'])) {
				$group_id_array = explode(',', $goods_info['group_id_array']);
				if (array_search($value, $group_id_array) !== false) {
					unset($group_id_array[ array_search($value, $group_id_array) ]);
				}
				$data['group_id_array'] = implode(',', $group_id_array);
			}
		}
		if (!empty($data)) {
			return $goods_model->save($data, [ 'goods_id' => $goods_id ]);
		}
	}
	
	public function getSelectClicks($goods_id){
		$goods_model = new NsGoodsModel();
		
		$res = $goods_model->getInfo(['goods_id'=>$goods_id]);
