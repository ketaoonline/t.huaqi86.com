...
class User extends BaseService
{
	 ...
	 * 拉取会员头像到本地
	 */
	public function pullOutUserImg($uid) {
		$condition = array(
			'uid' => $uid
		);
		
		$user_info = $this->user->getInfo($condition, $field = 'uid,user_headimg');

		$local_path = '';
		if (!empty($user_info['user_headimg'])) {
			$config = new Config();
			$upload_type = $config->getUploadType($this->instance_id);
			if($upload_type == 1){
				if (!file_exists('upload/user')) {
					$mode = intval('0777', 8);
					mkdir('upload/user', $mode, true);
				...
				$local_path = 'upload/user/' . time() . rand(111, 999) . '.png';
				$local_path = save_weixin_img($local_path, $user_info['user_headimg']);
			}else if($upload_type == 2){
				$qiniu = new QiNiu();
				$result = $qiniu->setQiniuUplaod("upload/" . $user_info['user_headimg'], "upload/" . $user_info['user_headimg']);
				if($result['code'] == 1){
	 ...
	 * 检测账号是否存在
	 */
	public function memberIsUserName($user_name)
	{
	 ...
	 * 根据openid查询手机号
	 */
	public function getMobileByOpenId($condition)
	{
		$user_info = $this->user->getInfo($condition, 'user_tel');
		return $user_info;
	}
	 ...
	 * 用户登录之后初始化数据
	 */
	private function initLoginInfo($user_info, $type = "")
	{
		$model = $this->getRequestModel();
		...
		if (!empty($type)) {
			...
			$member = new Member();
			$member_info = $member->getMemberDetail($this->instance_id, $user_info['uid']);
			$token = array(
				'uid' => $user_info['uid'],
				'request_time' => time()
			);
			$encode = $this->niuEncrypt(json_encode($token));
			Session::set('niu_member_detail', $member_info);
			...
			if (!empty($token_json)) {
				...
				if (!empty($token['openid'])) {
					...
					if (!$count) {
						...
						$wx_info_array = json_decode($token['info'], true);
						$wx_info_array['nickname'] = $this->filterStr($wx_info_array->nickname);
						$token['info'] = json_encode($wx_info_array);
	...
	public function qqLogin($qq)
	{
		...
		if (!empty($user_info)) {
			if ($user_info['user_status'] == 0) {
				...
				$this->initLoginInfo($user_info, "QQ");
				return 1;
			}
		} else
			return USER_NBUND;
	...
	public function wchatLogin($openid)
	{
		...
		if (!empty($user_info)) {
			if ($user_info['user_status'] == 0) {
				...
				$this->initLoginInfo($user_info, "WCHAT");
				return 1;
			}
		} else
			return USER_NBUND;
	...
	public function wchatUnionLogin($unionid)
	{
		...
		if (!empty($user_info)) {
			if ($user_info['user_status'] == 0) {
				...
				$this->initLoginInfo($user_info, "UNION");
				return 1;
			}
		} else
			return USER_NBUND;
