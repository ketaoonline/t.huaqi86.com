...
class OrderRefund extends Order
{
	...
	public function orderGoodsRefundAskfor($order_id, $order_goods_id, $refund_type, $refund_require_money, $refund_reason)
	{
		...
		try {
			...
			//已经操作过维权退款的,非操作完不可继续操作
			if(in_array($order_goods_info['refund_status'], [1,2,3,4,5,-2])){
				return -9;
			}
	...
	public function orderGoodsRefundFinish($order_id)
	{
		...
		try {
			...
			if ($refunding_count > 0) {
				...
				if ($order_info['order_type'] == 6 && $order_info['pay_money'] > 0) {
					$orderInfo->order_status = $this->getOrderStatus([ "order_type" => $order_info["order_type"] ])[7]['status_id']; //备货中
				} else {
	...
	public function orderGoodsRefundAgree($order_id, $order_goods_id)
	{
		...
		try {
			...
			$refund_status = $order_goods->getInfo(['order_goods_id' => $order_goods_id]);
			if($refund_status['refund_status'] != 1){
				return -6;
			}
...
// 				$goods->where(["goods_id" => $goods_id])->setInc('stock', $storage_num);
	...
	public function orderGoodsConfirmRefundAction($order_id, $order_goods_id, $refund_real_money, $refund_balance_money, $refund_trade_no, $refund_way, $refund_remark)
	{
		...
		try {
			...
			if (empty($refund_remark)) {
				...
				$refund_type_name = '';
				if (!empty($pay_type_info['type_name'])) {
					if ($pay_type_info['type_name'] != '支付宝') {
						$refund_type_name = str_replace('支付', '', $pay_type_info['type_name']);
					} else {
						$refund_type_name = $pay_type_info['type_name'];
					}
				}
				$remark = "订单编号:" . $order_info['order_no'] . "，退款方式为:[" . $refund_type_name . "]，退款金额:" . $refund_real_money . "元，退款余额：" . $refund_balance_money . "元";
				if ($freight > 0) {
	...
	public function orderGoodsCustomerServiceAskfor($order_goods_id, $refund_type, $refund_money, $refund_reason)
	{
		...
		try {
			...
			if ($order_info['is_virtual'] == 1) {
				...
				$verification_info = $order_vertify->getVirtualOrderGoods($order_info['order_no']);
				if ($verification_info == 1) {
					return -99;
				}
			}
	...
	public function orderGoodsCustomerAgree($id, $order_id, $order_goods_id)
	{
		...
		try {
			...
			$audit_status = $customer_service->getInfo(['id' => $id]);
			if($audit_status['audit_status'] != 1){
				return -6;
			}
	...
	public function orderCustomerConfirmRefund($id, $order_id, $order_goods_id, $refund_real_money, $refund_balance_money, $refund_way, $refund_remark)
	{
		...
		$refunded_money = $customer_service->getSum([ "order_id" => $order_id ], 'refund_real_money'); // 已退还的金额
	 ...
	 * @return void|Ambigous <number, unknown>
	 */
	public function orderGoodsCancelAfterSale($order_id, $order_goods_id){
		
		$customer_service = new NsCustomerServiceModel();
		
		$info = $customer_service->getInfo(['order_goods_id'=>$order_goods_id]);
		
		if($info['audit_status'] == 4 || $info['audit_status'] == 5){
			return;
		}
		
		$res = $customer_service->destroy(['order_goods_id'=>$order_goods_id]);
