...
class Notice extends BaseService
{
	 ...
	 * 添加通知记录
	 */
	public function createNoticeRecords($shop_id, $uid, $send_type, $send_account, $notice_title, $notice_context, $records_type, $send_config, $is_send, $template_id)
	{
	 ...
	 * 创建验证码发送记录
	 */
	public function createVerificationCodeRecords($shop_id, $uid, $send_type, $send_account, $send_config, $records_type, $notice_title, $notice_context, $send_message, $is_send, $template_id)
	{
		$notice_records_model = new NoticeRecordsModel();
		$data = array(
			"shop_id" => $shop_id,
		...
		$notice_records_model->save($data);
		$insert_id = $notice_records_model->id;
		Cache::clear('notice');
		return $insert_id;
	}
	...
	public function sendNoticeRecords()
	{
		...
		foreach ($notice_list as $notice_obj) {
			...
			if ($send_type == 1) {
				// 短信发送
				$this->noticeSmsSend($notice_obj["id"], $notice_obj["send_account"], $notice_obj["send_config"], $notice_obj["notice_context"], $notice_obj["template_id"]);
			} else {
				// 邮件发送
	 ...
	 * 发送短信
	 */
	private function noticeSmsSend($notice_id, $send_account, $send_config, $notice_params, $template_id)
	{
		...
		if (!empty($template_id)) {
		    $notice_template = new NoticeTemplateModel();
		    $template_info = $notice_template->getInfo(['template_id' => $template_id], 'template_content');
	...
	public function updateNoticeTemplate($shop_id, $template_type, $template_array, $notify_type)
	{
		...
		foreach ($template_data as $template_obj) {
			...
			$notification_mode = !empty($template_obj["notification_mode"]) ? $template_obj["notification_mode"] : '';
			$notice_template_model = new NoticeTemplateModel();
