...
class Shop extends BaseService
{
	 ...
	 * @return unknown
	 */
	public function getShopNavigationTemplateList($condition){
	    $template_model = new NsShopNavigationTemplateModel();
	    $template_list = $template_model->getQuery($condition);
	    return $template_list;
	}
	...
	public function getGoodsRecommend($page_index = 1, $page_size = 0, $condition = [], $order = '', $field = '*')
	{
		...
		$config = new Config();
		$system_config = $config->getSystemConfig($this->instance_id);
		$sort_type_arr = [
        	'default' => 'ng.sort '.$system_config['sort_type'].',ng.goods_id desc',
        	'sortdesc' => 'ng.sort desc,ng.goods_id desc',
        	'sortasc' => 'ng.sort asc,ng.goods_id desc',
        	'sales' => 'ng.sales desc,ng.goods_id desc',
        	'clicks' => 'ng.clicks desc,ng.goods_id desc'
        ];
		foreach ($list['data'] as $k => $v) {
		    $select_goods_order = $sort_type_arr['default'];
		    if (!empty($v['sort_type']) && isset($sort_type_arr[ $v['sort_type'] ])) {
		        $select_goods_order = $sort_type_arr[ $v['sort_type'] ];
		    }
			if ($v['type'] == 1) {
				//标签
				if (!empty($v['alis_id'])) {
					...
					$list['data'][ $k ]['goods_list'] = $goods->getRecommendGoodsList(1, $v['show_num'], $conditions, $select_goods_order);
				} else {
				...
				if (!empty($v['alis_id'])) {
					...
					$list['data'][ $k ]['name'] = '分类';
					$list['data'][ $k ]['goods_list'] = $goods->getRecommendGoodsList(1, $v['show_num'], [ 'ng.category_id_1|ng.category_id_2|ng.category_id_3' => $v['alis_id'], 'ng.state'=> 1 ], $select_goods_order);
				} else {
					$list['data'][ $k ]['type_name'] = '';
					$list['data'][ $k ]['name'] = '';
					$list['data'][ $k ]['goods_list'] = [];
				...
				if (!empty($v['alis_id'])) {
					...
					$list['data'][ $k ]['name'] = '品牌';
					$list['data'][ $k ]['goods_list'] = $goods->getRecommendGoodsList(1, $v['show_num'], [ 'ng.brand_id' => $v['alis_id'], 'ng.state'=> 1], $select_goods_order);
				} else {
					$list['data'][ $k ]['type_name'] = '';
					$list['data'][ $k ]['name'] = '';
					$list['data'][ $k ]['goods_list'] = [];
				...
				$list['data'][ $k ]['name'] = '推荐';
				$list['data'][ $k ]['goods_list'] = $goods->getRecommendGoodsList(1, $v['show_num'], [ 'ng.is_new' => 1, 'ng.state'=> 1 ], $select_goods_order);
			} else if ($v['type'] == 5) {
				$list['data'][ $k ]['type_name'] = '精品';
				$list['data'][ $k ]['name'] = '推荐';
				$list['data'][ $k ]['goods_list'] = $goods->getRecommendGoodsList(1, $v['show_num'], [ 'ng.is_recommend' => 1, 'ng.state'=> 1 ], $select_goods_order);
			} else if ($v['type'] == 6) {
				$list['data'][ $k ]['type_name'] = '热卖';
				$list['data'][ $k ]['name'] = '推荐';
				$list['data'][ $k ]['goods_list'] = $goods->getRecommendGoodsList(1, $v['show_num'], [ 'ng.is_hot' => 1, 'ng.state'=> 1 ], $select_goods_order);
			} else if ($v['type'] == 7) {
				if (!empty($v['alis_id'])) {
					$list['data'][ $k ]['goods_list'] = $goods->getRecommendGoodsList(1, $v['show_num'], [ 'ng.goods_id' => [ 'in', $v['alis_id'] ], 'ng.state'=> 1 ], $select_goods_order);
			...
			foreach ($data as $k => $v) {
				if ($k == "text") {
					...
					foreach ($v as $child_k => $child_v) {
						if (!empty($child_v)) {
							$condition = array();
							$page_size = $system_config['show_num'];
							...
							$order = $sort_type_arr['default'];
							if (!empty($child_v['sort_type']) && isset($sort_type_arr[ $child_v['sort_type'] ])) {
							    $order = $sort_type_arr[ $child_v['sort_type'] ];
							}
							$list = $product_model->getGoodsListNew(1, $page_size, $condition, $order);
							$data[ $k ][ $child_k ] = $list['data'];
						}
					}
	...
	public function getAdvPositionDetail($condition = [])
	{
		...
		if (!empty($info)) {
			$advs = $platform_adv->pageQuery(1, 0, [ 'ap_id' => $info['ap_id'] ], '', '*');
			$info['advs'] = $advs['data'];
		}
	 ...
	 * 设置首页浮动
	 */
	public function getAppletFloatConfig()
	{
		$cache = Cache::tag('config')->get("getAppletFloatConfig");
		$config_module = new ConfigModel();
		if (empty($cache)) {
			$info = $config_module->getInfo([
				'key' => 'APPLET_FLOAT_CONFIG',
				'instance_id' => 0
			], 'value');
			if (empty($info)) {
				$config_module = new ConfigModel();
				...
				$config_module->save($data);
				Cache::tag('config')->set("APPLET_FLOAT_CONFIG", $data);
				return $value;
			} else {
				$data = json_decode($info['value'], true);
				Cache::tag('config')->set("getAppletFloatConfig", $data);
				return $data;
			}
		} else {
			return $cache;
	 ...
	 * add，edd首页浮动
	 */
	public function setAppletFloatConfig($data)
	{
		Cache::tag('config')->set("APPLET_FLOAT_CONFIG", null);
		$value = $data;
		$config_module = new ConfigModel();
		$info = $config_module->getInfo([
			'key' => 'APPLET_FLOAT_CONFIG',
		...
		Cache::tag('config')->set("getAppletFloatConfig", $value);
		return $res;
	}
