...
class Promotion extends BaseService
{
	 ...
	 * 修改优惠券的状态
	 */
	public function updateCouponTypeClose($coupon_type_id, $status){
		$coupon_type = new NsCouponTypeModel();
		
		$res = $coupon_type->save(['status'=>$status], ['coupon_type_id'=>$coupon_type_id]);
	...
	{
		$coupon_type = new NsCouponTypeModel();
		$coupon_type_list = $coupon_type->pageQuery($page_index, $page_size, $condition, $order, 'coupon_type_id, coupon_name, money, count, max_fetch, at_least, need_user_level, range_type, start_time, end_time, create_time, update_time,is_show, term_of_validity_type, fixed_term, get_num, is_end, status');
	...
	{	
		...
		try {
			...
			$condition_type['end_time'] = array( [ 'LT', time() ], [ 'NEQ', 0 ] );
			$ns_coupon_type = new NsCouponTypeModel();
			$type_count = $ns_coupon_type->getCount($condition_type);

			if($type_count){
				$ns_coupon_type->save([ 'status' => 2 ], $condition_type);
			}
	...
	public function addPromotionGift($param)
	{
		...
		try {
			...
			if(!empty($param['sku_id'])){
				...
				$sku_info = $sku_model->getInfo(["sku_id"=>$param['sku_id']], "sku_name, picture");
				if($sku_info['sku_name'] != ''){
					$data_goods['goods_name'] = $goods_info['goods_name'] . '&nbsp;' .$sku_info['sku_name'];
				}else{
					$data_goods['goods_name'] = $goods_info['goods_name'];
				}
				if($sku_info['picture'] != 0){
					$data_goods['goods_picture'] = $sku_info['picture'];
				}else{
					$data_goods['goods_picture'] = $goods_info['picture'];
				}
			}else{
				$data_goods['goods_picture'] = $goods_info['picture'];
				$data_goods['goods_name'] = $goods_info['goods_name'];
			...
			$data_goods['gift_id'] = $gift_id;
			$data_goods['goods_id'] = $param["goods_id"];
			$data_goods['sku_id'] = $param["sku_id"];
	...
	public function updatePromotionGift($param)
	{
		...
		try {
			...
			if(!empty($param['sku_id'])){
				...
				$sku_info = $sku_model->getInfo(["sku_id"=>$param['sku_id']], "sku_name, picture");
				if($sku_info['sku_name'] != ''){
					$data_goods['goods_name'] = $sku_info['sku_name'];
				}else{
					$data_goods['goods_name'] = $goods_info['goods_name'];
				}
				if($sku_info['picture'] != 0){
			...
			$data_goods['gift_id'] = $param["gift_id"];
			$data_goods['goods_id'] = $param["goods_id"];
			$data_goods['sku_id'] = $param["sku_id"];
	...
	public function deletePromotionGift($gift_id)
	{
		...
		], "start_time,end_time,gift_id,status");
		// 开启事务
		$promotion_gift->startTrans();
		try {
			if (is_array($promotion_list) && count($promotion_list) > 0) {
				foreach ($promotion_list as $info) {
					// 未开始、已结束的赠品不能删除
					if ($info['start_time'] > time() || time() > $info['end_time'] || $info['status'] == 1) {
						$promotion_gift_goods = new NsPromotionGiftGoodsModel();
	...
	public function getPromotionGiftDetail($gift_id)
	{
		...
		if($gift_goods['sku_id'] == 0){
			...
			$sku_model = new NsGoodsSkuModel();
			$sku_info = $sku_model->getInfo(['sku_id'=>$gift_goods['sku_id']], "sku_id, goods_id, sku_name, price, stock, picture");
			...
			$pic_info['pic_cover'] = '';
			if($sku_info['picture'] != 0){
				$pic_info = $picture->get($sku_info['picture']);
			}else{
				$sku_info['picture'] = $goods_info['picture'];
				$pic_info = $picture->get($goods_info['picture']);
			}
				
			$goods_info['picture_info'] = $pic_info;
			$goods_info['gift_id'] = $gift_id;
			$goods_info['sku_id'] = $gift_goods['sku_id'];
	...
	public function userAchieveGift($param)
	{
		...
		try {
			...
			if (!empty($promotion_gift_goods_info['goods_id'])) {
				...
				$goods_sku_model = new NsGoodsSkuModel();
				if (!empty($promotion_gift_goods_info['sku_id'])) {
				    $sku_info = $goods_sku_model->getInfo([
				        'goods_id' => $promotion_gift_goods_info['goods_id'],
				        'sku_id' => $promotion_gift_goods_info['sku_id']
...
// 						    Db::table('ns_goods')->where('goods_id', $goods_item['goods_id'])->update([ 'promotion_price' => $goods_info['price'] ]);
	...
	public function getNewestDiscount()
	{
		...
		if (!empty($info)) {
			$result = $info;
			$promotion_discount_goods = new NsPromotionDiscountGoodsViewModel();
			$promotion_goods_list = $promotion_discount_goods->getViewQuery(1, 0, ['npdg.discount_id' => $info['discount_id'], 'ng.state'=>1 ], '');
			if (!empty($promotion_goods_list)) {
				foreach ($promotion_goods_list as $k => $v) {
					$picture = new AlbumPictureModel();
					if (!empty($v['picture'])) {
	...
	public function addUpdatePromotionGame($param)
	{
		...
		try {
			...
			$data = array(
				...
				'remark' => $param["remark"],
				//是否在首页展示
				'is_index_show' => $param["is_index_show"],
...
// 				$info = $promotion_games->pageQuery(1, 0, "", "game_id desc limit 1", "*");
	 ...
	 * 获取商品收藏详情
	 */
	public function getCollatingGoodsDetail($goods_id, $sku_id = "")
	{
		$goods = new Goods();
		...
		if (!empty($spec_list)) {
			...
			foreach ($spec_list as $k => $v) {
				foreach ($v["value"] as $t => $m) {
					...
					if(!empty($sku_id)){
						foreach ($sku_id_array as $key => $val){
							$sku_info = $sku_model->getInfo(['sku_id'=>$val]);					
							if($goods_id == $sku_info['goods_id']){	
																					
								$value = $m['spec_id'] . ":" . $m['spec_value_id'];
								$match = strstr($sku_info['attr_value_items'], $value);
								if ($match) {										
									$spec_list[ $k ]["value"][ $t ]["sku_type"] = 1;
								}else{
									$spec_list[ $k ]["value"][ $t ]["sku_type"] = 0;
								}
							}
						}
					}
	 ...
	 * 获取限时折扣商品列表
	 */
	public function getDialogGoodsList($goods_id_array, $discount_id, $type, $sku_id = ""){
		...
		if(!empty($sku_id)){
			$condition["sku_id"] = array(
					"in",
					$sku_id
			);
			$sku_model = new NsGoodsSkuModel();
			$list = $sku_model->pageQuery(1, 0, $condition, "create_date desc", "goods_id, sku_name, stock, promote_price, price");
			foreach ($list['data'] as $k => $v){
				$goods_info = $ns_goods->getInfo(['goods_id'=>$v['goods_id']], "goods_name");
				if($v['sku_name'] != ''){
					$list['data'][$k]['goods_name'] = $goods_info['goods_name'] .'&nbsp;'. $v['sku_name'];
				}else{
					$list['data'][$k]['goods_name'] = $goods_info['goods_name'];
				}
			}
	 ...
	 * 关闭赠品
	 */
	public function setGiftStatus($gift_id){
		$mansong_rule = new NsPromotionMansongRuleModel();
		$mansong_model = new NsPromotionMansongModel();
		$mansong_rule_list = $mansong_rule->getQuery(['gift_id'=>$gift_id]);
		if(!empty($mansong_rule_list)){
			foreach ($mansong_rule_list as $key => $val){
				$mansong_info = $mansong_model->getInfo(['mansong_id'=>$val['mansong_id']]);
				if($mansong_info['status'] == 1){
					return -1;
				}
			}
		}
		...
		$res = $gift_model->save(['status'=>1],['gift_id'=>$gift_id]);
	 ...
	 * 获取优惠券商品表
	 */
	public function getCouponGoodsList($coupon_type_id){
		$coupon_goods = new NsCouponGoodsModel();
		
		$list = $coupon_goods->getQuery(['coupon_type_id'=>$coupon_type_id]);
