...
class Auth extends User
{
	...
	public function getAuthList($pid)
	{
		...
		$list = $sys_module->where($condition)
			->order("sort")
			->column('module_id,module_name,module,controller,method,pid,url,is_menu,is_dev,icon_class,is_control_auth, icon_class');
	...
	{
		$admin_group_model = new AuthGroupModel();
		$group_info = $admin_group_model->getInfo(['is_system'=>1]);
		
		if ($this->is_admin || $group_info['group_id'] == $this->group_id) {
			return 1;
		} else {
	...
	public function getchildModuleQuery($moduleid)
	{
		...
		if (empty($module_list)) {
			...
			foreach ($new_list as $k => $v) {
				...
				} else if ($v['is_dev'] == 1 && config('app_debug')) {
				    $arrange_list[] = $new_list[ $k ];
				}
			}
	 ...
	 */
	public function breadCrumb($module_id, $module_array = []){
	    if (!empty($module_id)) {
    	    $module_info = $this->module->getInfo(['module_id' => $module_id], 'module_id,module_name,module,controller,method,pid,url');
    	    array_push($module_array, $module_info);
    	    if (!empty($module_info['pid'])) {
    	        return $this->breadCrumb($module_info['pid'], $module_array);
    	    } else {
    	        return array_reverse($module_array);
    	    }
	    }
	}
	 ...
	 * 更新管理员组的权限
	 */
	public function toUpdateRole(){
		
		$module_list = $this->module->getQuery("","*","module_id");
		
		$module_str = "";
		foreach ($module_list as $key => $val){
			 $module_str.= $val['module_id'].',';
		}
		$module_str = rtrim($module_str, ',');
		
		$res = $this->authgroup->save(["module_id_array"=>$module_str], ['is_system' => 1]);
	 ...
	 * 
	 */
	public function getUserGroupJump(){

		$auth_group_info = $this->getSystemUserGroupDetail($this->group_id);
		
		$group_arr = explode(",", $auth_group_info['module_id_array']);
		
		$sys_module = new SysModel();
		
		//查找第二级
		$module2_url = "";
		$module2_list = $sys_module->getQuery(["pid"=>$group_arr[0]]);
		foreach ($module2_list as $key => $val){
			if(strstr(','.$auth_group_info['module_id_array'].',', ','.$val['module_id'].',')){
				$module2_url = $val['url'];
				break;
			}
		}
