	...
	public function orderCreate($data)
	{
		...
		try {
			...
			if(!empty($data['custom_data'])){
				//添加订单自定义数据
				$custom_form = new NsCustomFormDataModel();
				$json_data = json_encode($data['custom_data']);
				$custom_data = [
						"cfd_data" => $json_data,
						"order_id" => $order_id,
						"create_time" => time()
	...
	public function getOrderGiftList($data)
	{
		...
		if (!empty($data["gift_id_array"])) {
			foreach ($data["gift_id_array"] as $k => $v) {
				$gift_goods_info = $promotion_gift_goods_model->getInfo([ 'gift_id' => $v ], "goods_id, sku_id");
				$goods_id = $gift_goods_info["goods_id"];
				
				$temp_array = [];
				$temp_array["shop_id"] = $data["shop_id"];
				$temp_array["num"] = 1;
				if($gift_goods_info['sku_id'] == 0){
					$goods_sku_info = $goods_sku_model->getInfo([ 'goods_id' => $goods_id ], "*");						
				}else{
					$goods_sku_info = $goods_sku_model->getInfo([ 'sku_id' => $gift_goods_info['sku_id']], "*");						
				}
		...
		if (!empty($promotion_array)) {
			foreach ($promotion_array as $promotion_k => $promotion_v) {
				...
				if ($promotion_v["promotion_type"] == 'MANJIAN') {
					$order_data['full_reduction'] = $promotion_v['discount_money'];
...
//         }
		...
		if (!empty($member_coupon_list)) {
			foreach ($member_coupon_list as $k => $coupon) {				
				...
				if ($type_info['range_type'] == 1) {
					...
					$goods_id_array = [];
					foreach ($coupon_goods_list as $key => $v) {
				...
				if($type_class == 1){
					$sort_arr[$k] = $coupon['money'];
					$sort_arra[$k] = $coupon['end_time'];
				}
			}
		}
		//优惠券价格倒序,到期日期倒序
		if (is_array($coupon_list) && count($coupon_list) > 1) array_multisort($sort_arr, SORT_DESC, $sort_arra, SORT_ASC, $coupon_list);
		return $coupon_list;
	}
	...
	public function dataCollation($data)
	{
        ...
        if ($data["order_config"]["order_online_pay"] == 1) {
		    array_push($pay_type, ["type_id" => 1,"type_name" => "在线支付"]);
		}
...
//                 $pickup_point_list["data"] = $this->pickupPointListSort($data["address"], $pickup_point_list["data"]);
			...
			//发票
			$invoice_info = [];
			if($data['order_config']['order_invoice_type'] == 1){
			...
			if ($data["order_config"]["order_delivery_pay"] == 1) {
			    array_push($pay_type, ["type_id" => 4,"type_name" => "货到付款"]);
			}
