...
//			runhook('Notify', 'registAfter', $params);
	...
	public function getMemberDetail($shop_id = '', $uid = '')
	{
		...
		if (!empty($uid)) {
			...
			$avg_order_money = 0;
			$order_money = $order->getSum([ 'buyer_id' => $user_info['uid'], 'pay_status' => 2, 'order_status' => [ [ 'neq', 5 ], [ 'neq', 0 ] ] ], 'pay_money');
			$order_count = $order->getCount([ 'buyer_id' => $user_info['uid'], 'pay_status' => 2, 'order_status' => [ [ 'neq', 5 ], [ 'neq', 0 ] ] ]);
			if ($order_count != 0) {
...
//	         $api_secure = $config->getApiSecureConfig();
	...
	public function getMemberAccount($uid)
	{
		...
		if (empty($account_info)) {
			...
			$account_info['member_cunsum'] = 0;
			$account_info['member_sum_point'] = 0;
		}
		// 购买次数
	...
	public function addMemberAccount($shop_id, $type, $uid, $num, $text)
	{
		...
		if ($num > 0) {
			hook('businessRechargeAfter', [ 'recharge_money' => $num, 'uid' => $uid ]);
		}
		return $retval;
	}
...
//				hook('memberWithdrawAuditAgree', [ 'id' => $id, 'type' => 'balance' ]);
	 ...
	 * 查询会员余额信息各项的总和
	 */
	public function getSelectBalanceSum($type)
	{
		$member_account_records_view = new NsMemberAccountRecordsViewModel();
		//商城订单支出
		$shancghen_sum = $member_account_records_view->getSum([ 'from_type' => 1, 'account_type' => $type], "number");
		//订单退还
		$order_sum = $member_account_records_view->getSum([ 'from_type' => 2, 'account_type' => $type ], "number");
		//兑换
		$duihuan_sum = $member_account_records_view->getSum([ 'from_type' => 3, 'account_type' => $type ], "number");
		//充值
		$chongzhi_sum = $member_account_records_view->getSum([ 'from_type' => 4, 'account_type' => $type ], "number");
		//签到
		$qiandao_sum = $member_account_records_view->getSum([ 'from_type' => 5, 'account_type' => $type ], "number");
		//分享
		$fenxiang_sum = $member_account_records_view->getSum([ 'from_type' => 6, 'account_type' => $type ], "number");
		//注册
		$zhuce_sum = $member_account_records_view->getSum([ 'from_type' => 7, 'account_type' => $type ], "number");
		//提现
		$tixian_sum = $member_account_records_view->getSum([ 'from_type' => 8, 'account_type' => $type ], "number");
		//提现退还
		$titui_sum = $member_account_records_view->getSum([ 'from_type' => 9, 'account_type' => $type ], "number");
		//调整
		$tiaozheng_sum = $member_account_records_view->getSum([ 'from_type' => 10, 'account_type' => $type ], "number");
		$yue_zong = [
	...
	public function getMemberCouponTypeList($shop_id, $uid)
	{
		...
		$coupon_type_list_two = $coupon_type_model->getQuery([ 'is_show' => 1, 'shop_id' => $shop_id, 'term_of_validity_type' => 1, "status" => 0 ], '*', 'create_time desc');
		...
		if (!empty($coupon_type_list)) {
			...
			foreach ($coupon_type_list as $k => $v) {
				...
				$coupon_type_list[ $k ]['money'] = intval($v['money']);
				
				$coupon_type_list[ $k ]['at_least'] = intval($v['at_least']);
	...
	{
		$result = $this->verifyMemberLevelExist($data);//检测等级条件是否可用
		if ($result["code"] < 0) {
			return $result;
		}
		$member_level = new NsMemberLevelModel();
		...
		return success($member_level->level_id);
	}
	 ...
	 */
	public function verifyMemberLevelExist($data, $condition = [])
	{
		...
		$member_level = new NsMemberLevelModel();
		$count = $member_level->getCount([ "$field" => $value ]);
		if ($count > 0) {
			if (!empty($condition)) {
				$info = $member_level->getInfo($condition);
				if ($info[ $field ] == $data[ $field ] && $count == 1) {
					return success();
				}
			}
			
			return error("当前" . $desc . "在其他会员等级的升级条件下已存在,请核对后重新设置");
		} else {
			return success();
		}
	}
	...
	public function checkMemberLevel($shop_id, $user_id)
	{
		...
		switch ($config_info["type"]) {
			case 1:
				...
				$last_level_1 = $member_level_model->getFirstData([ "min_integral" => [ "<=", $member_account_obj["member_sum_point"] ] ], "level desc");
				if (!empty($last_level_1)) {
					if ($last_level_1['level'] > $level_info['level']) {
						$data_1 = array(
							"level_id" => $last_level_1["level_id"],
							"uid" => $user_id
						);
						$this->updateMemberLevelAction($data_1);
					}
			...
			case 2:
				...
				$last_level_2 = $member_level_model->getFirstData([ "quota" => [ "<=", $money ] ], "level desc");
				
				if (!empty($last_level_2)) {
					if ($last_level_2['level'] > $level_info['level']) {
						$data_2 = array(
							"level_id" => $last_level_2["level_id"],
							"uid" => $user_id
						);
						$this->updateMemberLevelAction($data_2);
					}
			...
			case 3:
				...
				$last_level_3 = $member_level_model->getFirstData([ "order_num" => [ "<=", $order_num ] ], "level desc");
				
				if (!empty($last_level_3)) {
					if ($last_level_3['level'] > $level_info['level']) {
						$data_3 = array(
							"level_id" => $last_level_3["level_id"],
							"uid" => $user_id
						);
						$this->updateMemberLevelAction($data_3);
					}
	...
	public function getGuessMemberLikes($page_index, $page_size)
	{
		...
		if (!empty($history)) {
			...
			$condition['category_id'] = $category_id['category_id'];
		}
		
		$goods_list = $goods_model->pageQuery($page_index, $page_size, $condition, "create_time desc", "goods_id,goods_name,price,point_exchange_type,point_exchange,promotion_price,picture");
		if (!empty($goods_list)) {
	 ...
	 */
	public function giveCoupon($uid, $coupon_type_id)
	{
		$uid_arr = explode(',', $uid);
		$coupon_type_id_arr = explode(',', $coupon_type_id);
		if (!empty($uid_arr) && !empty($coupon_type_id_arr)) {
			try {
				foreach ($uid_arr as $uid) {
					foreach ($coupon_type_id_arr as $coupon_type_id) {
						$this->memberGetCoupon($uid, $coupon_type_id, 5);
					}
				}
				$res = [ 'code' => 1, 'message' => '发放成功' ];
			} catch (\Exception $e) {
