...
use data\model\SysSeoConfigModel;
...
class Config extends BaseService
{
	...
	public function getHotsearchConfig($instanceid)
	{
		...
		if (empty($cache)) {
			...
			if (empty($info) || empty($info['value'])) {
				Cache::tag('config')->set("getHotsearchConfig" . $instanceid, 'unset');
				return;
			} else {
	...
	public function getDefaultSearchConfig($instanceid)
	{
		...
		if (empty($cache)) {
			...
			if (empty($info) || empty($info['value'])) {
				Cache::tag('config')->set("getDefaultSearchConfig" . $instanceid, 'unset');
				return;
			} else {
				$data = json_decode($info['value'], true);
	...
	{
		$sms_list = hook('smsconfig', [ 'instance_id' => $instanceid ]);
		$data = array(
			'value' => array(
				'appKey' => '',
				'secretKey' => '',
	...
	public function getAppletCategoryDisplay($instanceid)
	{
		...
		$res = '{"template":"1","style":1,"is_img":"0","is_use":"0"}';
...
// 		}
	...
	public function getRegisterAndVisitInfo($shop_id)
	{
		...
		$mobile_config = $this->getMobileMessage($shop_id);
		$reg_config['mobile_config'] = $mobile_config;
		return $reg_config;
	}
	 ...
	 * 注册访问设置
	 */
	public function setRegisterAndVisit($shop_id, $is_register, $register_info, $name_keyword, $pwd_len, $pwd_complexity, $terms_of_service, $is_requiretel, $is_use, $is_login)
	{
		$value_array = array(
	...
	{
		$cache = Cache::tag('config')->get("ShortcutMenu" . $this->instance_id . $this->uid);
		if (!empty($cache)) {
			return $cache;
		}
		$model = new SysShortcutMenuModel();
		$list = $model->getViewList(1, 0, ['ssm.uid'=>$this->uid], '');
		Cache::tag('config')->set("ShortcutMenu" . $this->instance_id . $this->uid, $list);
		return $list;
	}
	/*************************************************系统后台菜单结束******************************************************/
	...
	public function getWebSiteColorScheme($flag)
	{
		...
		if (!empty($info) && !empty($info['value'])) {
			$info['value'] = json_decode($info['value'], true);
			Cache::tag('config')->set("getWebSiteColorScheme" . $this->instance_id . "_" . $flag, $info['value']);
			return $info['value'];
		} else {
			Cache::tag('config')->set("getWebSiteColorScheme" . $this->instance_id . "_" . $flag, 'unset');
			return;
		}
	}
	...
	public function getApiSecureConfig()
	{
		...
		if (empty($cache)) {
			...
			if (empty($info['value'])) {
				...
				$res = $config_module->save($data);
				Cache::tag('config')->set("getApiSecureConfig", $value);
				return $value;
			} else {
				$data = json_decode($info['value'], true);
	 ...
	 */
	public function editWapEntrance($params, $port)
	{
		Cache::clear('entrance_config');
		$data = [
			'title' => $params['title'],
	 ...
	 */
	public function getSystemConfig($instance_id)
	{
		$cache = Cache::tag('config')->get('getSystemConfig' . $instance_id);
		if (!empty($cache)) return $cache;
		
		// 商品列表商品展示数
		$show_num_info = $this->getConfig($instance_id, 'SYSTEM_SHOW_NUM');
		// 系统排序方式
		$sort_type_info = $this->getConfig($instance_id, 'SYSTEM_SORT_TYPE');
		
		$data = [
			'show_num' => !empty($show_num_info['value']) ? $show_num_info['value'] : PAGESIZE,
			'sort_type' => !empty($sort_type_info['value']) ? $sort_type_info['value'] : 'desc'
		];
		Cache::tag('config')->set('getSystemConfig' . $instance_id, $data);
		return $data;
	}
	...
	{
		Cache::tag('config')->set('getSystemConfig' . $data['instance_id'], null);
		$array = [];
		if (isset($data['show_num'])) {
			array_push($array, [
		...
		if (isset($data['sort_type'])) {
			array_push($array, [
				'instance_id' => $data['instance_id'],
				'key' => 'SYSTEM_SORT_TYPE',
				'value' => $data['sort_type'],
				'desc' => '系统排序方式',
				'is_use' => 1
			]);
		}
	 ...
	 * @param unknown $instance_id
	 */
	public function getSystemRechargeConfig($instance_id)
	{
	    $cache = Cache::tag('config')->get('getSystemRechargeConfig' . $instance_id);
	    if (!empty($cache)) return $cache;
	
	    $data = $this->getConfig($instance_id, 'SYSTEM_RECHARGE');
	    if (!empty($data)) {
	    ...
	    Cache::tag('config')->set('getSystemRechargeConfig' . $instance_id, $data);
	    return $data;
	}
	 ...
	 * @param unknown $data
	 */
	public function setSystemRechargeConfig($data)
	{
	    Cache::tag('config')->set('getSystemRechargeConfig' . $data['instance_id'], null);
	    $array = [
	        [
	            'instance_id' => $data['instance_id'],
	            'key' => 'SYSTEM_RECHARGE',
	 ...
	 * 获取seo配置列表
	 */
	public function getSysSeoConfigList($page_index = 1, $page_size = 0, $condition = [], $order = "seo_id asc", $field = "*"){
		
		$seo_model = new SysSeoConfigModel();
		
		$seo_list = $seo_model->pageQuery($page_index, $page_size, $condition, $order, $field);
	 ...
	 * 根据主键获取seo配置
	 */
	public function getSeoConfigInfo($condition){
		$seo_model = new SysSeoConfigModel();
		
		$info = $seo_model->getInfo($condition);
	 ...
	 * 设置seo
	 */
	public function updateSeoConfig($seo_id, $seo_title, $seo_meta, $seo_desc, $seo_other){
		...
		$res = $seo_model->save($data, ['seo_id'=>$seo_id]);
