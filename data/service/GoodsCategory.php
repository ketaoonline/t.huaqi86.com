...
class GoodsCategory extends BaseService
{
	...
	public function getCategoryTreeUseInShopIndex()
	{
		...
		$cache = Cache::tag("niu_goods_category")->get("getCategoryTreeUseInShopIndex".$system_config['sort_type']);
		if (empty($cache)) {
			$goods_category_model = new NsGoodsCategoryModel();
			$goods_category_one = $goods_category_model->getQuery([
				'level' => 1,
				'is_visible' => 1
			], 'category_id, category_name,short_name,pid,category_pic', 'sort '.$system_config['sort_type']);
