	...
	public function orderGoodsDelivery($order_id, $order_goods_id_array)
	{
		...
		try {
			...
			foreach ($order_goods_id_array as $k => $order_goods_id) {
				...
				$order_goods_info['order_goods_id']=$order_goods_id;
				hook('orderGoodsDelivery', $order_goods_info);
			}
...
//			));
	...
	public function pickedUpAuditorConfirmPickup($order_id, $auditor_id, $buyer_name, $buyer_phone)
	{
		...
		try {
			...
			$picked_up_auditor_info = $ns_picked_up_auditor_view->getViewInfo([ 'npua.uid' => $auditor_id ]);
...
//			));
	...
	public function ordersClose()
	{
		...
		try {
			...
			if (!empty($order_list) || !empty($presell_order_list)) {
	...
	public function createNewOutTradeNoReturnBalance($order_id)
	{
		...
		if (!empty($order_info)) {
			...
			if (!empty($pay_info) && $pay_info['balance_money'] > 0) {
				...
				$member_account = new MemberAccount();
				$member_account->addMemberAccountData(0, 2, $order_info['buyer_id'], 0, $pay_info['balance_money'], 1, $order_id, "订单再次发起支付，返还上次支付锁定余额");
				$data = array(
