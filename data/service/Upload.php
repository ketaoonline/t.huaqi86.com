...
class Upload extends BaseService
{
	 ...
	 */
	public function image($file, $file_upload_path, $thumb_type = '', $size = 30000000, $watermark = false)
	{
...
//				$filetype = $upload->getFileType($upload_data);//获取文件类型
				...
				if ($this->is_watermark) $this->imageWater($upload_path . "/" . $file_name . "." . $extend_name, $upload_path . "/" . $file_name . "." . $extend_name);
...
//				$filetype = $upload->getFileType($upload_data);//获取文件类型
	...
	public function imageWater($file_name, $warer_name)
	{
		...
		try {
			...
			if ($type == 1) {
				...
				$res = $image->water($this->imgWatermark, $locate, $alpha);
	 ...
	 * 云存储调用
	 */
	public function fileStoreHaiBao($upload_path)
	{
		$upload = new UploadExtend('');
	
		$file_size = $upload->getFileSize($upload_path);
	
		if ($this->upload_type == 2) {
			$qiniu = new QiNiu();
		
			$res = $qiniu->deleteQiNiuImage($upload_path, "", 2);

			$result = $qiniu->setQiniuUplaod($upload_path, $upload_path);
	
			$delete_file_name = substr($upload_path, strlen($this->upload_path . "/"));
			$this->deleteFile($delete_file_name);
			$result["size"] = $file_size;
			return $result;
		}
	
		return [ "code" => 1, 'path' => $upload_path, "size" => $file_size ];
	}
