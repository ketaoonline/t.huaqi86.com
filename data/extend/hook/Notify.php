...
class Notify
{
    ...
    private function getShopNotifyInfo($shop_id){
        ...
        $sms_list = hook('smsconfig', ['instance_id' => $shop_id]);
        if (!empty($sms_list)) {
            foreach ($sms_list as $item) {
                if ($item['is_use']) {
                    $this->sms_config = $item;
                    break;
                } 
            }
        }
    ...
    public function registAfter($params=null){
        ...
        if(!empty($mobile) && $this->mobile_is_open==1){
            ...
            if(!empty($template_obj) && $template_obj["is_enable"]==1){
                ...
                $this->createNoticeSmsRecords($template_obj, $shop_id, $params["user_id"], $mobile, $sms_params, "注册成功短信通知", 17, 0, $template_obj['template_id']);
            }
        }
        //邮箱验证
        if(!empty($email) && $this->email_is_open==1){
            ...
            if(!empty($template_obj) && $template_obj["is_enable"]==1){
                ...
                $this->createNoticeEmailRecords($shop_id, $params["user_id"], $email, $send_title, $content, 5, 0, $template_obj['template_id']);
            }
        }
        //模板消息
    ...
    public function registSmsValidation($params=null){
        ...
        $this->result['record_id'] = $this->createVerificationCodeRecords($template_obj, $shop_id, 0, 1, $mobile, 9, "用户注册短信验证码", json_encode($sms_params), $this->result["message"], $send_result, $template_obj['template_id']);
        return $this->result;
    }
    ...
    public function registEmailValidation($params=null){
        ...
        $send_result = $this->result["code"] < 0 ? -1 : 1;
        $this->result['record_id'] = $this->createVerificationCodeRecords($template_obj, $shop_id, 0, 2, $email, 10, "用户注册邮箱验证码", json_encode(['number' => $rand]), $this->result["message"], $send_result, $template_obj['template_id']);
    ...
    public function orderDelivery($params=null){
        ...
        if(count($order_goods_str)>0){
            ...
            if(!empty($mobile) && $this->mobile_is_open==1){
                ...
                if(!empty($template_obj) && $template_obj["is_enable"]==1){
                    $this->createNoticeSmsRecords($template_obj, $shop_id, $buyer_id, $mobile, $sms_params, "订单发货短信发送", 5, 0, $template_obj['template_id']);
                }
            }
            // 邮件发送
            if (!empty($user_obj)) {
                ...
                if (! empty($email) && $this->email_is_open == 1) {
                    ...
                    if (! empty($template_obj) && $template_obj["is_enable"] == 1) {
                        ...
                        $this->createNoticeEmailRecords($shop_id, $buyer_id, $email, $send_title, $content, 5, 0, $template_obj['template_id']);
                    }
                }
            }
            ...
            if(addon_is_exit("NsWxtemplatemsg"))
            {
                ...
                $url = str_replace('pay.php', 'index.php', $url);
                $param = [
                    'uid'      => $buyer_id,
    ...
    public function orderComplete($params=null){
        ...
        if (! empty($mobile) && $this->mobile_is_open == 1) {
            ...
            if (! empty($template_obj) && $template_obj["is_enable"] == 1) {
                $this->createNoticeSmsRecords($template_obj, $shop_id, $buyer_id, $mobile, $sms_params, "订单确认短信发送", 2, 0, $template_obj['template_id']);
            }
        }
        // 邮件发送
        $user_model = new UserModel();
        ...
        if (! empty($user_obj)) {
            ...
            if (! empty($email) && $this->email_is_open == 1) {
                ...
                if (! empty($template_obj) && $template_obj["is_enable"] == 1) {
                    ...
                    $send_title=str_replace("{主订单号}", $order_no, $send_title);
                    $send_title=str_replace("{订单金额}", $order_money, $send_title);
                    $this->createNoticeEmailRecords($shop_id, $buyer_id, $email, $send_title, $content, 2, 0, $template_obj['template_id']);
                }
            }
        }
        //模板消息
    ...
    public function orderPay($params=null){
        ...
        if(!empty($mobile) && $this->mobile_is_open == 1){
            ...
            if(!empty($template_obj) && $template_obj["is_enable"]==1){
                $this->createNoticeSmsRecords($template_obj, $shop_id, $buyer_id, $mobile, $sms_params, "订单付款成功通知", 3, 0, $template_obj['template_id']);
            }
        }
        $user_model=new UserModel();
        $user_obj=$user_model->get($buyer_id);
        if(!empty($user_obj)){
            ...
            if(!empty($email) && $this->email_is_open==1){
                ...
                if(!empty($template_obj) && $template_obj["is_enable"]==1){
                    ...
                    $send_title=str_replace("{订单金额}", $order_money, $send_title);
                    $send_title=str_replace("{商品金额}", $goods_money, $send_title);
                    $this->createNoticeEmailRecords($shop_id, $buyer_id, $email, $send_title, $content, 3, 0, $template_obj['template_id']);
                }
            }
        }
        //模板消息
    ...
    public function orderCreate($params=null){
        ...
        if(!empty($mobile) && $this->mobile_is_open==1){
            ...
            if(!empty($template_obj) && $template_obj["is_enable"]==1){
                $this->createNoticeSmsRecords($template_obj, $shop_id, $buyer_id, $mobile, $sms_params, "订单创建成功通知", 4, 0, $template_obj['template_id']);
            }
        }
        // 邮件发送
        $user_model = new UserModel();
        ...
        if (! empty($user_obj)) {
            ...
            if (! empty($email) && $this->email_is_open == 1) {
                ...
                if (! empty($template_obj) && $template_obj["is_enable"] == 1) {
                    ...
                    $send_title = str_replace("{订单金额}", $order_money, $send_title);
                    $send_title = str_replace("{商品金额}", $goods_money, $send_title);
                    $this->createNoticeEmailRecords($shop_id, $buyer_id, $email, $send_title, $content, 4, 0, $template_obj['template_id']);
                }
            }
        }
        //模板消息
    ...
    public function forgotPassword($params=null){
        ...
        if($send_type=="email"){
            ...
            $this->result['record_id'] = $this->createVerificationCodeRecords($template_obj, $shop_id, 0, 2, $send_param, 11, "找回密码邮箱验证码", json_encode(['number' => $rand]), $this->result["message"], $send_result, $template_obj['template_id']);
            ...
            $send_result = $this->result["code"] < 0 ? -1 : 1;
            $this->result['record_id'] = $this->createVerificationCodeRecords($template_obj, $shop_id, 0, 1, $send_param, 12, "找回密码短信验证码", json_encode($sms_params), $this->result["message"], $send_result, $template_obj['template_id']);
        }
        return $this->result;
    }
    /**
    ...
    public function bindMobile($params=null){
        ...
        $send_result = $this->result["code"] < 0 ? -1 : 1;
        $this->result['record_id'] = $this->createVerificationCodeRecords($template_obj, $shop_id, 0, 1, $mobile, 13, "绑定手机号短信验证码", json_encode($sms_params), $this->result["message"], $send_result, $template_obj['template_id']);
    ...
    public function bindEmail($params=null){
        ...
        $send_result = $this->result["code"] < 0 ? -1 : 1;
        $this->result['record_id'] = $this->createVerificationCodeRecords($template_obj, $shop_id, 0, 2, $email, 14, "绑定邮箱邮箱验证码", json_encode(['number' => $rand]), $this->result["message"], $send_result, $template_obj['template_id']);
        return $this->result;
    }
    ...
    public function orderRemindBusiness($params=null){
        ...
        if(!empty($out_trade_no)){
            ...
            if(!empty($email_array[0]) && $template_email_obj["is_enable"] == 1){
                ...
                $content = str_replace("{收件人}", $order_detial['receiver_name'], $content);
                $content = str_replace("{收货地址}", $order_detial['receiver_address'], $content);
                ...
                $send_title = str_replace("{收件人}", $order_detial['receiver_name'], $send_title);
                $send_title = str_replace("{收货地址}", $order_detial['receiver_address'], $send_title);
                foreach ($email_array as $v){
                    $this->createNoticeEmailRecords($shop_id, 0, $v, $send_title, $content, 7, 0, $template_email_obj['template_id']);
                }
            }
            //短信提醒
            ...
            if(!empty($mobile_array[0]) && $template_sms_obj["is_enable"] == 1){
                $sms_params=array(
                    ...
                    "orderno"=>$order_detial['order_no'],
                    "ordermoney"=>$order_detial['order_money'],
                	"receivername"=>$order_detial['receiver_name'],
                	"receiveraddress"=>$order_detial['receiver_address']
                );
                foreach ($mobile_array as $v){
                    $this->createNoticeSmsRecords($template_sms_obj, $shop_id, 0, $v, $sms_params, "订单提醒-商家通知", 7, 0, $template_sms_obj['template_id']);
                }
            }
            ...
            if(addon_is_exit("NsWxtemplatemsg"))
            {
            		...
            		$url = '';
            		$keyword = array();
            		$keyword["keyword1"] = $order_detial["order_no"];//订单编号
            		$keyword["keyword2"] = getTimeStampTurnTime($order_detial["create_time"]);//付款时间
            		$keyword["keyword3"] = $order_detial["order_money"];//订单金额
    ...
    public function orderRefundBusiness($params=null){
        ...
        $shop_id = $params['shop_id'];
        $refund_require_money = $params['refund_require_money'];
        $result="";
        $user_name = "";
        if(!empty($order_id)){
            //获取订单详情
            ...
            if(!empty($email_array[0]) && $template_email_obj["is_enable"] == 1){
                ...
                $content = str_replace("{主订单号}", $order_detial['order_no'], $content);
                $content = str_replace("{订单金额}", $order_detial['order_money'], $content);
                $content = str_replace("{退款金额}", $refund_require_money, $content);
                $send_title = $template_email_obj["template_title"];
                $send_title = str_replace("{商场名称}", $this->shop_name, $send_title);
                $send_title = str_replace("{用户名称}", $order_detial['user_name'], $send_title);
                $send_title = str_replace("{主订单号}", $order_detial['order_no'], $send_title);
                $send_title = str_replace("{订单金额}", $order_detial['order_money'], $send_title);
                $send_title = str_replace("{退款金额}", $refund_require_money, $send_title);
                foreach ($email_array as $v){
                    $this->createNoticeEmailRecords($shop_id, 0, $v, $send_title, $content, 6, 0, $template_email_obj['template_id']);
                }
            }
            //短信提醒
            ...
            if(!empty($mobile_array[0]) && $template_sms_obj["is_enable"] == 1){
                $sms_params=array(
                    ...
                    "orderno"=>$order_detial['order_no'],
                    "ordermoney"=>$order_detial['order_money'],
                	"refundmoney"=>$refund_require_money
                );
                foreach ($mobile_array as $v){
                    $this->createNoticeSmsRecords($template_sms_obj, $shop_id, 0, $v, $sms_params, "订单退货提醒-商家通知", 6, 0, $template_sms_obj['template_id']);
                }
            }
            ...
            if(addon_is_exit("NsWxtemplatemsg"))
            {
                    ...
                    $keyword["keyword2"] = $refund_require_money;
    ...
    public function rechargeSuccessBusiness($params=null){
        ...
        if(!empty($out_trade_no)){
            ...
            if(!empty($email_array[0]) && $template_email_obj["is_enable"] == 1){
                ...
                foreach ($email_array as $v){
                    $this->createNoticeEmailRecords($shop_id, 0, $v, $send_title, $content, 8, 0, $template_email_obj['template_id']);
                }
            }
            //短信提醒
            ...
            if(!empty($mobile_array[0]) && $template_sms_obj["is_enable"] == 1){
                ...
                foreach ($mobile_array as $v){
                    $this->createNoticeSmsRecords($template_sms_obj, $shop_id, 0, $v, $sms_params, "余额充值-商家通知", 8, 0, $template_sms_obj['template_id']);
                }
            }
            ...
            if(addon_is_exit("NsWxtemplatemsg"))
            {
                ...
                $url = '';
            	$keyword = array();
            	$keyword["keyword1"] = $user_name;
            	$keyword["keyword2"] = "";
            	$keyword["keyword3"] = $order_payment['pay_money'] . '元';//充值金额
            
            	$param = array(
            			"uid" => $uid,
            			"template_no" => 'OPENTM411530050',
            			"first" => '',
            			"url" => $url,
            			"keyword" => $keyword,
            			"template_code" => $params["tag"]
     ...
     * 用户充值余额用户提醒
     */
    public function memberBalanceRechargeSuccess($params=null){
        $this->getShopNotifyInfo(0);
        $out_trade_no = $params["out_trade_no"];//订单号
        $shop_id = $params['shop_id'];
        $user = new UserModel();
        ...
        if(!empty($out_trade_no)){
            ...
            if(!empty($user_email) && $template_email_obj["is_enable"] == 1){
                ...
                $send_title = str_replace("{用户名称}", $user_name, $send_title);
                $send_title = str_replace("{充值金额}", $order_payment['pay_money'], $send_title);
                $this->createNoticeEmailRecords($shop_id, $user_info["uid"], $user_email, $send_title, $content, 1, 0, $template_email_obj['template_id']);
            }
            //短信提醒
            ...
            if(!empty($user_tel) && $template_sms_obj["is_enable"] == 1){
                ...
                $this->createNoticeSmsRecords($template_sms_obj, $shop_id, $user_info["uid"], $user_tel, $sms_params, "用户余额充值", 1, 0, $template_sms_obj['template_id']);
            }
        }
    ...
    public function openGroupNoticeUser($params = []){
        ...
        if(!empty($pintuan_info) && !empty($user_info)){
            ...
            if(!empty($template_email_obj) && $template_email_obj["is_enable"] == 1 && !empty($user_info['user_email'])){
                ...
                // 添加到发送记录表
                $this->createNoticeEmailRecords($shop_id, $pintuan_info['group_uid'], $email, $send_title, $content, 0, 0, $template_email_obj['template_id']);
            }
            ...
            if(!empty($template_sms_obj) && $template_sms_obj["is_enable"] == 1 && !empty($user_info['user_tel'])){
                ...
                $this->createNoticeSmsRecords($template_sms_obj, $shop_id, $pintuan_info['group_uid'], $user_info['user_tel'], $sms_params, "用户拼团发起通知", 0, 0, $template_sms_obj['template_id']);
            }
    ...
    public function addGroupNoticeUser($params = []){
        ...
        if(!empty($pintuan_info) && !empty($user_info)){
            ...
            if(!empty($template_email_obj) && $template_email_obj["is_enable"] == 1 && !empty($user_info['user_email'])){
                ...
                $send_title = str_replace("{拼团类型}", $pintuan_info['tuangou_type_name'], $send_title);
                // 添加到发送记录表
                $this->createNoticeEmailRecords($shop_id, $pintuan_info['group_uid'], $user_info['user_email'], $send_title, $content, 0, 0, $template_email_obj['template_id']);
            }
            ...
            if(!empty($template_sms_obj) && $template_sms_obj["is_enable"] == 1 && !empty($user_info['user_tel'])){
                ...
                $this->createNoticeSmsRecords($template_sms_obj, $shop_id, $pintuan_info['group_uid'], $user_info['user_tel'], $sms_params, "用户参与拼团通知", 0, 0, $template_sms_obj['template_id']);
            }
        ...
        if(!empty($pintuan_info) && !empty($user_list)){
            ...
            if(!empty($template_email_obj) && $template_email_obj["is_enable"] == 1){
                ...
                foreach ($user_list as $item){
                    ...
                    // 添加到发送记录表
                    $this->createNoticeEmailRecords($shop_id, $item['buyer_id'], $item['user_email'], $send_title, $content, 0, 0, $template_email_obj['template_id']);
                }
            }
            ...
            if(!empty($template_sms_obj) && $template_sms_obj["is_enable"] == 1){
                ...
                foreach ($user_list as $item){
                    if($params['type'] == "success"){
                        ...
                        $this->createNoticeSmsRecords($template_sms_obj, $shop_id, $item['buyer_id'], $item['user_tel'], $sms_params, "用户拼团成功通知", 0, 0, $template_sms_obj['template_id']);
                    }elseif($params['type'] == "fail"){
                        $this->createNoticeSmsRecords($template_sms_obj, $shop_id, $item['buyer_id'], $item['user_tel'], $sms_params, "用户拼团失败通知", 0, 0, $template_sms_obj['template_id']);
                    }
                }
            }
            //模板消息
    ...
    public function openGroupNoticeBusiness($params = []){
        ...
        if(!empty($pintuan_info) && !empty($user_info)){
            ...
            if(!empty($template_email_obj) && $template_email_obj["is_enable"] == 1 && !empty($email_array[0])){
                ...
                // 添加到发送记录表
                foreach ($email_array as $v){
                    $this->createNoticeEmailRecords($shop_id, $pintuan_info['group_uid'], $v, $send_title, $content, 0, 0, $template_email_obj['template_id']);
                }
            }
            ...
            if(!empty($template_sms_obj) && $template_sms_obj["is_enable"] == 1 && !empty($mobile_array[0])){
                ...
                foreach ($mobile_array as $v){
                    $this->createNoticeSmsRecords($template_sms_obj, $shop_id, $pintuan_info['group_uid'], $v, $sms_params, "用户发起拼团商家通知", 0, 0, $template_sms_obj['template_id']);
                }
            }
    ...
    public function groupBookingSuccessBusiness($params = []){
        ...
        if(!empty($pintuan_info)){
            ...
            if(!empty($template_sms_obj) && $template_sms_obj["is_enable"] == 1 && !empty($mobile_array[0])){
                ...
                foreach ($mobile_array as $v){
                    $this->createNoticeSmsRecords($template_sms_obj, $shop_id, $pintuan_info['group_uid'], $v, $sms_params, "拼团成功商家通知", 0, 0, $template_sms_obj['template_id']);
                }
            }
            ...
            {
            	$order_model = new NsOrderModel();
            	$order_info = $order_model->getInfo(['tuangou_group_id'=>$pintuan_info['group_id']], "order_no");
            	
            	$uid = "bind_openid";
            	$url = '';
            	$keyword = array();
            	$keyword["keyword1"] = $order_info["order_no"];
            	$keyword["keyword2"] = $pintuan_info['goods_name'];
            	$keyword["keyword3"] = $pintuan_info['tuangou_money'];
            	$keyword["keyword4"] = $pintuan_info['tuangou_num'] . '人团';
    ...
    public function bargainLaunchUser($params = []){
        ...
        if(!empty($bargain_info) && !empty($user_info)){
            ...
            if(!empty($template_email_obj) && $template_email_obj["is_enable"] == 1 && !empty($user_info['user_email'])){
                ...
                // 添加到发送记录表
                $this->createNoticeEmailRecords($shop_id, $bargain_info['uid'], $user_info['user_email'], $send_title, $content, 0, 0, $template_email_obj['template_id']);
            }
            ...
            if(!empty($template_sms_obj) && $template_sms_obj["is_enable"] == 1 && !empty($user_info['user_tel'])){
                ...
                $this->createNoticeSmsRecords($template_sms_obj, $shop_id, $bargain_info['uid'], $user_info['user_tel'], $sms_params, "用户砍价发起通知", 0, 0, $template_sms_obj['template_id']);
            }
            //模板消息
            if(addon_is_exit("NsWxtemplatemsg"))
            {
        ...
        if(!empty($bargain_info) && !empty($user_info)){
            ...
            if(!empty($template_sms_obj) && $template_sms_obj["is_enable"] == 1 && !empty($user_info['user_tel'])){
                ...
                if($params['type'] == 'success'){
                    ...
                    $this->createNoticeSmsRecords($template_sms_obj, $shop_id, $bargain_info['uid'], $user_info['user_tel'], $sms_params, "用户砍价成功通知", 0, 0, $template_sms_obj['template_id']);
                }elseif($params['type'] == 'fail'){
                    $this->createNoticeSmsRecords($template_sms_obj, $shop_id, $bargain_info['uid'], $user_info['user_tel'], $sms_params, "用户砍价失败通知", 0, 0, $template_sms_obj['template_id']);
                }
            }
            ...
            if(addon_is_exit("NsWxtemplatemsg"))
            {
                ...
                $keyword = array();
                $keyword["keyword1"] = $bargain_info['goods_name'];//商品名称
                $keyword["keyword2"] = number_format(round($bargain_info['goods_money'] - $bargain_info['bargain_money'], 2), 2);//当前价 $bargain_info['goods_money'] - $bargain_info['bargain_money'];//当前价
                if($params['type'] == 'success'){
                    $notice = "砍价成功通知";
                }elseif($params['type'] == 'fail'){
                    $notice = "砍价失败通知";
        ...
        if(!empty($bargain_info) && !empty($user_info)){
            ...
            if(!empty($template_email_obj) && $template_email_obj["is_enable"] == 1 && !empty($email_array[0])){
                ...
                foreach ($email_array as $email){
                    $this->createNoticeEmailRecords($shop_id, $bargain_info['uid'], $email, $send_title, $content, 0, 0, $template_email_obj['template_id']);
                }
            }
            ...
            if(!empty($template_sms_obj) && $template_sms_obj["is_enable"] == 1 && !empty($mobile_array[0])){
                ...
                foreach ($mobile_array as $mobile){
                    $this->createNoticeSmsRecords($template_sms_obj, $shop_id, $bargain_info['uid'], $mobile, $sms_params, "用户砍价发起商家通知", 0, 0, $template_sms_obj['template_id']);
                }
            }
    ...
    public function bargainSuccessBusiness($params = []){
        ...
        if(!empty($bargain_info) && !empty($user_info)){
            ...
            if(!empty($template_sms_obj) && $template_sms_obj["is_enable"] == 1 && !empty($mobile_array[0])){
                ...
                foreach ($mobile_array as $mobile){
                    $this->createNoticeSmsRecords($template_sms_obj, $shop_id, $bargain_info['uid'], $mobile, $sms_params, "用户砍价成功商家通知", 0, 0, $template_sms_obj['template_id']);
                }
            }
            ...
            if(addon_is_exit("NsWxtemplatemsg"))
            {
            	...
            	$keyword = array();
            	$keyword["keyword1"] = $bargain_info['goods_name'];//商品名称
            	$keyword["keyword2"] = number_format(round($bargain_info['goods_money'] - $bargain_info['bargain_money'], 2), 2);//当前价
     ...
     */
    private function createNoticeSmsRecords($template_obj, $shop_id, $buyer_id, $mobile, $sms_params, $title, $records_type, $is_send = 0, $template_id){
        if($this->sms_config["is_use"] != 1){
            return;
        }
        //过滤参数
        if($template_obj['template_content'] != '') {
            foreach ($sms_params as $k => $v) {
                if(strpos($template_obj['template_content'],'{'.$k.'}') === false){
                    unset($sms_params[$k]);
                }
            }
        }
        ...
        $notice_service->createNoticeRecords($shop_id, $buyer_id, 1, $mobile, $title, json_encode($sms_params), $records_type, json_encode($send_config), $is_send, $template_id);
    }
     ...
     * @param unknown $records_type
     */
    private function createNoticeEmailRecords($shop_id, $buyer_id, $mobile, $send_title, $content, $records_type, $is_send = 0, $template_id){
        ...
        $notice_service->createNoticeRecords($shop_id, $buyer_id, 2, $mobile, $send_title, $content, $records_type, json_encode($send_config), $is_send, $template_id);
    }
     ...
     */
    private function createVerificationCodeRecords($template_obj, $shop_id, $uid, $send_type, $send_account, $records_type, $notice_title, $notice_context, $send_message, $is_send, $template_id)
    {
        $notice_service = new Notice();
        if ($send_type == 1) {
            // 短信
        ...
        $res = $notice_service->createVerificationCodeRecords($shop_id, $uid, $send_type, $send_account, json_encode($send_config), $records_type, $notice_title, $notice_context, $send_message, $is_send, $template_id);
        return $res;
    }
    ...
    public function applyRefund($params){
        ...
        if($order_goods_info["refund_status"] == 1 ){
            ...
            if(!empty($email) && $template_email_obj["is_enable"] == 1){
                ...
                $this->createNoticeEmailRecords(0, $order_info["buyer_id"], $email, $send_title, $content, 0, 0, $template_email_obj['template_id']);
            }
            //短信提醒
            ...
            if(!empty($mobile) && $template_sms_obj["is_enable"] == 1){
                ...
                $this->createNoticeSmsRecords($template_sms_obj, 0, $order_info["buyer_id"], $mobile, $sms_params, "申请退款-用户通知", 0, 0, $template_sms_obj['template_id']);
    ...
    public function refundResult($params){
        ...
        if(!empty($email) && $template_email_obj["is_enable"] == 1)
        {
            ...
            $this->createNoticeEmailRecords(0, $uid, $email, $send_title, $content, 0, 0, $template_email_obj["template_id"]);
        }
        //短信提醒
        ...
        if(!empty($mobile) && $template_sms_obj["is_enable"] == 1){
            ...
            $this->createNoticeSmsRecords($template_sms_obj, 0, $uid, $mobile, $sms_params, "退款通过-用户通知", 0, 0, $template_sms_obj['template_id']);
    ...
    public function withdrawApply($params){
        ...
        if(!empty($mobile) && $template_sms_obj["is_enable"] == 1){
            ...
            $this->createNoticeSmsRecords($template_sms_obj, 0, $uid, $mobile, $sms_params, "提现申请提醒-用户通知", 0, 0, $template_sms_obj['template_id']);
    ...
    public function withdrawResult($params){
        ...
        if(!empty($mobile) && $template_sms_obj["is_enable"] == 1){
            ...
            $this->createNoticeSmsRecords($template_sms_obj, 0, $uid, $mobile, $sms_params, "提现审核结果-用户通知", 0, 0, $template_sms_obj['template_id']);
        }
        //模板消息
        if(addon_is_exit("NsWxtemplatemsg"))
        {
    ...
    public function promoterApply($params){
        ...
        if(!empty($mobile) && $template_sms_obj["is_enable"] == 1){
            ...
            $this->createNoticeSmsRecords($template_sms_obj, 0, $uid, $mobile, $sms_params, "分销商申请-用户通知", 0, 0, $template_sms_obj['template_id']);
        }
        //模板消息
        if(addon_is_exit("NsWxtemplatemsg"))
        {
    ...
    public function orderDistribution($params){
        ...
        //         $email_array = explode(',', $this->shop_notify_config["email"]);//获取要提醒的人
        if(!empty($email) && $template_email_obj["is_enable"] == 1 && $commission_money > 0)
        {
            $content = $template_email_obj["template_content"];
            $content = str_replace("{商场名称}", $this->shop_name, $content);
            $content = str_replace("{主订单号}", $order_no, $content);
        ...
        //         $mobile_array = explode(',', $this->shop_notify_config["mobile"]);//获取要提醒的人
        if(!empty($mobile) && $template_sms_obj["is_enable"] == 1 && $commission_money > 0){
            $sms_params=array(
                "shopname" => $this->shop_name,
                "ordermoney" => $order_money,
            ...
            $this->createNoticeSmsRecords($template_sms_obj, 0, $uid, $mobile, $sms_params, "分销订单提成-用户通知", 0, 0, $template_sms_obj['template_id']);
        }
        //模板消息
        if(addon_is_exit("NsWxtemplatemsg") && $commission_money > 0)
        {
            $url = "";
            $keyword = array();
    ...
    public function promoterAudit($params){
        ...
        if(!empty($mobile) && $template_sms_obj["is_enable"] == 1){
            ...
            $this->createNoticeSmsRecords($template_sms_obj, 0, $uid, $mobile, $sms_params, $params['title'], 0, 0, $template_sms_obj['template_id']);
        }
        //模板消息
        if(addon_is_exit("NsWxtemplatemsg"))
        {
