    ...
    */
    ...
    public function update_file_unzip($download_zip_path, $download_file_path, $download_update_file)
    {
        ...
        try {
             ...
             * 检索文件开始路径
             */
            $file_start_path=$download_update_file."/niushop_b2c_fx/";
        } catch (\Exception $e) {
            $upgrade_code=-1;
    ...
    public function execute_upgrade_sql($download_update_file, $version_no){
        $is_lock=$download_update_file."/niushop_fx_".$version_no."_patch.lock";
        if(!file_exists($is_lock)){
            fopen($is_lock, "w");
            $sqlfile = $download_update_file."/niushop_fx_".$version_no."_patch.sql";
