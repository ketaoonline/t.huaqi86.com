...
class WchatOauth
{
	...
	public function getAppletLiveRoom($start = 0, $limit = 10) {
	    $url = "http://api.weixin.qq.com/wxa/business/getliveinfo?access_token=%s";
	    $data = ['start' => $start, 'limit' => $limit];
        $re = $this->applet_get_url_return($url, json_encode($data));
        return json_decode($re, true);
    }

    public function getAppletLiveRoomPlayBack($room_id, $start = 0, $limit = 10) {
        $url = "http://api.weixin.qq.com/wxa/business/getliveinfo?access_token=%s";
        $data = ['action' => 'get_replay', 'room_id' => $room_id,'start' => $start, 'limit' => $limit];
        $re = $this->applet_get_url_return($url, json_encode($data));
        return json_decode($re, true);
    }
