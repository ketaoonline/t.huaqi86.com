...
function check() {
	...
	if (login_captcha == '' && !$('#login_captcha').parents('.log-cont').hasClass('hide')) {
		toast(lang_login.verification_code_cannot_be_null);
		return false;
	}
	if (login_captcha != '' && login_captcha != undefined && !$('#login_captcha').parents('.log-cont').hasClass('hide')) {
	...
	api("System.Login.login", {username: username, password: password}, function (res) {
		...
		if (data["code"] >= 0) {
			...
			member_sub = false;
			toast(data["message"]);
			if ($('#login_captcha').parents('.log-cont').hasClass('hide') && data.error_num > $('#hide_error_num').val()) {
				$('#login_captcha').parents('.log-cont').removeClass('hide');
			}
			$(".verifyimg").attr("src", __URL(SHOPMAIN + "/captcha?tag=1") + '&send=' + Math.random());
		}
	});
