$(function () {
				...
				if (list.length > 0) {
					...
					for (i in list) {
						...
						if (item.image != "") {
							...
							h += '<ul class="pics" id="gallery">';
							for (var ei = 0; ei < evaluate_img_arr.length; ei++) {
								h += '<li><img onclick="imgEnlarge(\''+ evaluate_img_arr[ei] +'\')" class="comment-pic" src="' + __IMG(evaluate_img_arr[ei]) + '" alt="用户评论" ></li>';
							}
							h += '</ul>';
						}
						...
						if (item['again_content'] != '') {
							...
							if (item['again_image'] != '') {
								...
								for (var key in imgs_arr) {
									h += '<li><img onclick="imgEnlarge(\''+ imgs_arr[key] +'\')" src="' + __IMG(imgs_arr[key]) + '"  data-index="' + key + '" class="comment-pic"></li>';
								}
								h += '</ul>';
							}
		...
		if (uid != "") {
			...
			if (pay_way == "add_cart") {
				...
				api('System.Goods.addCart', {"cart_detail": JSON.stringify(cart_detail)}, function (res) {
					...
					if (data.code > 0) {
						toast("加入购物车成功");
						api("System.Goods.cartCount",{}, function(res){
							var	cart = res.data;
							$(".have-cart-count").show();
							$(".have-cart-count").html(cart);
						});
	...
	function changeProductSku() {
		...
		if ($("input[name='product_sku']").length > 1) {
			$("input[name='product_sku']").each(function () {
				...
				$('.widgets-cover .sku-list-wrap a.selected:not(.disabled)').each(function () {
					...
					if ($.inArray($(this).data("id"), value.split(";")) != -1) {
						match_sku_count++;
		...
		if($("#hidden_order_type").val() == 6){
			price = current_sku.attr("data-original-price");
			$(".main .original-price span").html("￥"+price);
		}
		
		// 如果是拼团活动
		if ($("#hidden_order_type").val() == 4) {
			var price = $('[name="pintuan_sku"][value="'+ sku_id +'"]').data('pintuan-price');
			$(".product-bottom-bar .sku-wrap .header .main .price").text("¥" + price);
		}
...
function countDown() {
	var end_time = $(".product-discount .countdown .txt").attr('data-value'),
		start_time = $(".product-discount .countdown .txt").attr('data-start'),
		_date = ($("#current_time").val() / 1000),
		surplus_time = end_time - _date,		
		timer = setInterval(function () {
			if(start_time > _date){
				surplus_time = start_time - _date;
			}else{
				surplus_time = end_time - _date;
			}
			if (surplus_time > 0) {
...
function imgEnlarge(img){
	$(".img-enlarge").show();
	$(".img-enlarge img").attr("src", __IMG(img));
}
