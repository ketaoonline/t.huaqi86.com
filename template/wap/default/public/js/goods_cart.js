...
$(function () {
	...
	$(".num").blur("input propertychange", function () {
		...
		$($cart).parent(".ui-number").next("[name='succ_amount']").text(num);
...
//检测商品限购，是否允许购买
function getGoodsPurchaseRestrictionForCurrentUser(cart_id, goods_id, num, callBack) {
	api('System.Goods.goodsPurchaseRestriction', {"goods_id": goods_id, "num": num, "cart_id": cart_id}, function (res) {
		if (res.code == 0) {
...
var Cart = {
	changeBar: function (type, skuId, obj, goods_id) {
		...
		var is_allow = true;
		if (type == '+') {
			getGoodsPurchaseRestrictionForCurrentUser(skuId, goods_id, 1, function (call) {
				if (call.code == 0) {
					is_allow = false;
					num = default_num;
					toast(call.message);
		...
		calculated_price(num, obj, goods_id, skuId);
...
//计算阶梯优惠
function calculated_price(num, obj, goodsid, skuId) {
	...
	if (arr.length > 0) {
		for (var i = 0; i < arr.length; i++) {
			...
			for (var v = 0; v < item.length; v++) {
				if (num >= item[v]['quantity'] && item[v]['goods_id'] == goodsid && skuId == item[v]['cart_id']) {
					price -= item[v]['price'];
					break;
				}
			}
