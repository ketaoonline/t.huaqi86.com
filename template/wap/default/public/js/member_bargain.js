	...
	api("NsBargain.Bargain.myBargain", {"page_index": page}, function (res) {
		...
		if (data['data'].length == 0) {
			...
			for (var i = 0; i < data['data'].length; i++) {
				...
				html += '<footer>';
				if (curr.status == 1) {
					html += '<span>还差<strong class="ns-text-color">' + Math.floor((curr.goods_money - curr.bargain_money) * 100) / 100 + '</strong>元，<time data-end-time="' + curr.end_time + '">剩余00:00:00</time></span>';
					...
					html += '</li>';
					html += '</ul>';
					html += '<button class="btn-order-info ns-border-color-gray" onclick="location.href=\'' + __URL(APPMAIN + '/Goods/bargainLaunch?launch_id=' + curr.launch_id) + '\'" style="margin-left:5px">查看砍价详情</button>';
					if(curr.order_status === 0){
						html += '<button class="btn-order-info ns-border-color-gray" onclick="location.href=\''+ __URL(APPMAIN +'/order/lists?status=0') +'\'">去支付</button>';
					}
				}
				html += '</footer>';
				html += '</li>';
