...
$(function () {
	...
	$('.invoice-cont .is-need-invoice li').click(function (event) {
		...
		if ($(this).attr('data-value') == 0) {
			...
			$('.invoice-cont.invoice .form-horizontal').addClass('hide');
			$('.invoice-cont.invoice .form-horizontal [name]').val('');
		} else {
			vue.isNeedInvoice = true;
			$('.invoice-cont.invoice .form-horizontal').removeClass('hide');
		}
		order.calculate();
	});
	...
	this.getValue = function () {
		...
		if ($('.invoice-cont .is-need-invoice li.active').attr('data-value') == 1) {
			...
			if($('input[name="invoice_type"]:checked').val() == 1){
				...
				data.buyer_invoice = invoiceFormObj.find('[name="invoice_title"]').val() + '$' + invoiceFormObj.find('li.active').text() + '$' + invoiceFormObj.find('[name="taxpayer_identification_number"]').val() + '$' + invoiceFormObj.find('[name="opening_Bank"]').val() + '$' + invoiceFormObj.find('[name="bank_account"]').val() + '$' + invoiceFormObj.find('[name="business_address"]').val() + '$' + invoiceFormObj.find('[name="business_tel"]').val();
			}
		} else {
			data.buyer_invoice = '';
		}
	...
	this.verify = function () {
		...
		if ($('.invoice-cont .is-need-invoice li.active').attr('data-value') == 1) {
			...
			if($('input[name="invoice_type"]:checked').val() == 2){
				if ($('.invoice-cont [name="opening_Bank"]').val().search(/[\S]+/)) {
					show('请输入开户银行');
					return false;
				}
				if ($('.invoice-cont [name="bank_account"]').val().search(/[\S]+/)) {
					show('请输入银行账号');
					return false;
				}
				if ($('.invoice-cont [name="business_address"]').val().search(/[\S]+/)) {
					show('请输入企业地址');
					return false;
				}
				if ($('.invoice-cont [name="business_tel"]').val().search(/[\S]+/)) {
					show('请输入企业电话');
					return false;
				}
			}
		...
		$("form [name]").each(function(index, el){
			array.push({value : $(el).val(), is_fill : $(el).attr("data-fill"), data_name : $(el).attr("data-name"), data_type : $(el).attr("data-type")})
		...
		for (i = 0; i < array.length; i ++){
			if(array[i]['is_fill'] == 1 && array[i]['value'] == ""){
				show(array[i]['data_name'] + '不能为空');
				return false;
			}
			if(array[i]['data_type'] == 2 && isNaN(array[i]['value'])){
				show(array[i]['data_name'] + '只能是数字');
				return false;
			}
		}
		...
		if($("input[name='radio_text']").length>0){
			$(".form-horizontal input[name='radio_text']").each(function(index, el){
				
				custom_data.push({
					data_name : $(el).attr("data-name"),
					value : $(el).next().next().next().find("input[type='radio']:checked").attr("data-name"),
		...
		if($("input[name='check_text']").length>0){
			$(".form-horizontal input[name='check_text']").each(function(index, el){
				var check_text = [];
