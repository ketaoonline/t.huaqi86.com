...
function login() {
	...
	if (password == null || password == "") {
		show(lang_login.please_input_pd);
		$("#password").focus();
		return;
	}
	if (is_flag) return false;
	is_flag = true;
	if ($("#hidden_verify_pc").val() == 1 && !$('#vertification').parents('.input-control').hasClass('hide')) {
	...
	api("System.Login.login", {
		...
		if (data['code'] < 0) {
			show(data['message']);
			$('.login-type.account .verifyimg').attr("src", __URL(SHOPMAIN + '/captcha?tag=1&send=' + Math.random()));
			if ($('#vertification').parents('.input-control').hasClass('hide') && data.error_num > $('#hide_error_num').val()) {
				$('#vertification').parents('.input-control').removeClass('hide');
			}
			is_flag = false;
		} else {
			$.ajax({
