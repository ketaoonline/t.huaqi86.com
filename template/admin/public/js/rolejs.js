...
function AllCheckBoxClick(event) {
    ...
    if ($(eventSrc).attr("dir") == "four") { 
        if ($(eventSrc).attr("checked") == "checked") {
            ...
            $(eventSrc).parents('ul.second').find("[dir='son']").parent().addClass('selected');
            //为三级添加选中状态
            $(eventSrc).parents('li.third-stage').find("[dir='sonson']").prop("checked",true);
            $(eventSrc).parents('li.third-stage').find("[dir='sonson']").parent().addClass('selected');
        } else {
            //如果四级数量归零，则相应三级去除选中状态
            var parentObj = $(eventSrc).parents('ul.fourth-stage').find("[dir='four']:checked");
            var num = parentObj.length;
            if (num == 0) {
                $(eventSrc).parents('li.third-stage').find("[dir='sonson']").prop("checked",false);
                $(eventSrc).parents('li.third-stage').find("[dir='sonson']").parent().removeClass('selected');
            }
